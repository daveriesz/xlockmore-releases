...
vertex_dir(ModeInfo * mi, fringe_node_c * vertex, unsigned side)
{
	...
	if (MI_WIN_IS_VERBOSE(mi)) {
		(void) printf("Weirdness in vertex_dir (this has been reported)\n");
		for (i = 0; i < 5; i++)
			(void) printf("v2->fived[%d]=%d, vertex->fived[%d]=%d\n",
...
init_penrose(ModeInfo * mi)
{
	...
	if (tp->fringe.nodes != 0 || tp->forced.first != 0) {
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Weirdness in init_penrose()\n");
			(void) printf("tp->fringe.nodes = 0 && tp->forced.first = 0\n");
		}
...
check_vertex(ModeInfo * mi, fringe_node_c * vertex, tiling_c * tp)
{
	...
	if (vertex->rule_mask == 0) {
		tp->done = True;
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Dislocation occured!\n");
		}
...
delete_vertex(ModeInfo * mi, fringe_node_c * vertex, tiling_c * tp)
{
	if (tp->fringe.nodes == vertex) {
		tp->done = True;
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Weirdness in delete_penrose()\n");
			(void) printf("tp->fringe.nodes == vertex\n");
		}
		MI_PAUSE(mi) = CELEBRATE;
	}
	if (vertex->list_ptr != 0) {
...
alloc_vertex(ModeInfo * mi, angle_c dir, fringe_node_c * from, tiling_c * tp)
{
	...
	if (v == 0) {
		tp->done = True;
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Weirdness in alloc_vertex()\n");
			(void) printf("v = 0\n");
		}
		MI_PAUSE(mi) = CELEBRATE;
	}
	 ...
	 fringe_node_c * vertex, unsigned side, vertex_type_c vtype)
{
	...
	if (fc & FC_BAG) {
		tp->done = True;
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Weirdness in add_tile()\n");
			(void) printf("fc = %d, FC_BAG = %d\n", fc, FC_BAG);
		}
	}
	if (side == S_LEFT) {
		if (right == 0)
...
add_forced_tile(ModeInfo * mi, forced_node_c * node)
{
	...
	if (n <= 0) {
		tp->done = True;
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Weirdness in add_forced_tile()\n");
			(void) printf("n = %d\n", n);
		}
	}
...
add_random_tile(fringe_node_c * vertex, ModeInfo * mi)
{
	...
	if (n <= 0) {
		tp->done = True;
		if (MI_WIN_IS_VERBOSE(mi)) {
			(void) printf("Weirdness in add_random_tile()\n");
			(void) printf("n = %d\n", n);
		}
	}
	no_good = 0;
