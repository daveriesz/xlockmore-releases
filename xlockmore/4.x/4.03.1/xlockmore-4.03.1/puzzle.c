...
setupmovedelta(ModeInfo * mi)
{
	...
	if (pp->box) {
		...
		(void) XGetWindowAttributes(display, parent, &pwa);
		x = pp->nextcol * pp->boxsize.x + pp->randompos.x + 1;
		y = pp->nextrow * pp->boxsize.y + pp->randompos.y + 1;
		w = pp->boxsize.x - 2;
		h = pp->boxsize.y - 2;
		/* Test to if viewable and on screen else a BadMatch is generated */
		if (wa.map_state == IsViewable &&
		    (pwa.x + wa.x + x >= 0) && (pwa.y + wa.y + y >= 0) &&
		    (pwa.x + wa.x + x + w < DisplayWidth(display, screen)) &&
		    (pwa.y + wa.y + y + h < DisplayHeight(display, screen))) {
			pp->box = XGetImage(display, window, x, y, w, h, AllPlanes, ZPixmap);
			XFlush(display);
		}
	}
#if 0
