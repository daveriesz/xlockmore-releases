...
static const char sccsid[] = "@(#)xlock.c	4.00 97/01/01 xlockmore";
...
#if defined( USE_LOGOUT_BUTTON ) || defined( USE_AUTO_LOGOUT )
...
mode_info(int scrn, Window window, int iconic)
{
	...
	MI_COLORMAP(mi) = DefaultColormap(dsp, scrn);
...
#else
	char       *buffer = (char *) malloc(strlen(home) + 32);
