...
static const char sccsid[] = "@(#)triangle.c	4.00 97/01/01 xlockmore";
...
draw_atriangle(ModeInfo * mi, XPoint * p, int y_0, int y_1, int y_2, double dinv)
{
	...
	if (MI_NPIXELS(mi) > 2) {	/* color */
		...
		if (dmax == 0) {
			...
			color = NUMCOLORS -
				(int) ((double) NUMCOLORS / M_PI_2 * atan(dinv * (dmax - dmin)));
		}

		XSetForeground(display, gc, MI_PIXEL(mi, color % NUMCOLORS));
