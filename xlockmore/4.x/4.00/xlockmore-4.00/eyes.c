...
static const char sccsid[] = "@(#)eyes.c	4.00 97/01/01 xlockmore";
...
unpaint_fly(ModeInfo * mi, Fly * f)
{
	...
	if (MI_WIN_IS_ICONIC(mi)) {
		...
		XSetForeground(display, ep->flyGC, MI_WIN_BLACK_PIXEL(mi));
		XSetTSOrigin(display, ep->flyGC, f->oldx, f->oldy);
		XFillRectangle(display, window, ep->flyGC, f->oldx, f->oldy,
			       f->width, f->height);
	}
}
