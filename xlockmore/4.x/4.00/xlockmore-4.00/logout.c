...
static const char sccsid[] = "@(#)logout.c	4.00 97/01/01 xlockmore";
...
#if defined( USE_AUTO_LOGOUT ) || defined( USE_LOGOUT_BUTTON ) || defined( USE_BOMB )
 ...
 * on it's merry way (only applies if USE_AUTO_LOGOUT or USE_LOGOUT_BUTTON
 * is defined.)
 */
...
#ifdef _SGI_SOURCE /* What is a good switch for SGI/IRIX */
	(void) finish();
...
/* PURIFY 3.2 on Solaris2 and PURIFY 4.0 on SunOS4 reports an uninitialized
   memory read on the next line */
	pwp = getpwuid(uid);
...
#endif /* USE_AUTO_LOGOUT || USE_LOGOUT_BUTTON */

#endif /* USE_AUTO_LOGOUT || USE_LOGOUT_BUTTON || USE_BOMB */
