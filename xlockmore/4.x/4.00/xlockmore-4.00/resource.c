...
static const char sccsid[] = "@(#)resource.c	4.00 97/01/01 xlockmore";
...
#if defined( USE_AUTO_LOGOUT ) || defined( USE_LOGOUT_BUTTON )
extern int  fullLock();
...
#ifdef USE_LOGOUT_BUTTON
#ifdef ALLOW_NO_LOGOUT_BUTTON
#define DEF_BTN   	"5"	/* minutes until logout button appears */
#endif
...
static OptionStruct opDesc[] =
{
	...
	{"-version", "print version number (if >= 4.00)"},
...
#endif
#ifdef USE_AUTO_LOGOUT
	{"-forceLogout minutes", "number of minutes until auto logout"},
#endif
#ifdef USE_LOGOUT_BUTTON
#ifdef ALLOW_NO_LOGOUT_BUTTON
	{"-logoutButton minutes", "number of minutes until logout button appears"},
#endif
...
int         logoutButton;
#else
int         logoutButton = LO_BUTTON_TIME;
#endif
...
#ifdef USE_LOGOUT_BUTTON
#ifdef ALLOW_NO_LOGOUT_BUTTON
{(caddr_t *) & logoutButton, "logoutButton", "LogoutButton", DEF_BTN, t_Int},
#endif
...
Version(void)
{
	(void) fprintf(stderr, "XLock version %s\n", VERSION);
	exit(0);
}
     ...
     char *name, char *classname, int valueType, char *def, caddr_t * valuep)
{
	...
	switch (valueType) {
		case t_String:
			{
/* PURIFY 3.2 on Solaris2 and PURIFY 4.0 on SunOS4 reports a memory leak on
   the next line. I think its faked out */
