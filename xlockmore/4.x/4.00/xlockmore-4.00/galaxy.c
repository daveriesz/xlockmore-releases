...
static const char sccsid[] = "@(#)galaxy.c	4.00 97/01/01 xlockmore";
...
/* PURIFY 3.2 on Solaris2 and PURIFY 4.0 on SunOS4 reports a memory leak on
   the next line */
		...
		for (j = 0; j < gt->nstars; ++j) {
			...
			st->color = COLORSTEP * gt->galcol + j % COLORSTEP;
	...
	for (i = 0; i < gp->ngalaxies; ++i) {
		...
		for (j = 0; j < gp->galaxies[i].nstars; ++j) {
			...
			if (st->px >= gp->clip.left &&
			    st->px <= gp->clip.right - gp->star_size &&
			    st->py >= gp->clip.top &&
			    st->py <= gp->clip.bottom - gp->star_size) {
				XSetForeground(display, gc, MI_WIN_BLACK_PIXEL(mi));
				drawStar(st->px, st->py, gp->star_size);
			}
