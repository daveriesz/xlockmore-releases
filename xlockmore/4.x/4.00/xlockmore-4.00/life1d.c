...
static const char sccsid[] = "@(#)life1d.c	4.00 97/01/01 xlockmore";
...
#if 0
static unsigned char lcau21_rules[][8] =
{
...
static unsigned char lcau31_rules[][27] =
{
...
static unsigned char lcau41_rules[][64] =
{
...
GetRule(life1dstruct * lp, int i)
{
	...
	if (totalistic) {
		...
		for (j = 0; j < sum_size; j++) {
			lp->nextstate[sum_size - 1 - j] = (unsigned char) (code / pow);
...
init_life1d(ModeInfo * mi)
{
	...
	if (lp->nextstate == NULL)
		lp->nextstate = (unsigned char *) malloc(maxsum_size *
