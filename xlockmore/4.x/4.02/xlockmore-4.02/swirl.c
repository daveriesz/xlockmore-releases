...
/* 
   vTemplateWant.class = VisualClassWanted; if (VisualClassWanted == -1)
   info_list = XGetVisualInfo(display, VisualScreenMask | VisualDepthMask,
   &vTemplate, &n); else info_list = XGetVisUalInfo(display, VisualScreenMask
   | VisualDepthMask | VisualClassMask, &vTemplateWant, &n); */
	...
	*default_info = info_list;
	if (VisualClassWanted == -1) {
		...
		for (i = 0; i < n; i++, (*default_info)++)
			if ((*default_info)->depth >= 24 &&
			    (*default_info)->screen == screen &&
			    (*default_info)->class == VisualClassWanted)
				break;
		if (i == n) {
			*default_info = info_list;
			for (i = 0; i < n; i++, (*default_info)++)
	...
	if (MI_WIN_IS_VERBOSE(mi)) {
		...
		(void) fprintf(stderr, "class %s, ",
			       ...
			       nameOfVisualClass((*default_info)->c_class));
#else
			       nameOfVisualClass((*default_info)->class));
#endif
...
#if defined(__cplusplus) || defined(c_plusplus)
	swirl->fixed_colourmap = ((default_info->c_class == StaticGray) ||
...
draw_point(ModeInfo * mi, SWIRL_P swirl)
{
	...
	/* update the screen */
/* PURIFY 3.0a on SunOS4 reports a 256 byte memory leak on the next line. */
