...
static const char sccsid[] = "@(#)sproingies.c	4.2 97/03/20 xlockmore";
...
GLuint      build_s1_1(int wireframe);
GLuint      build_s1_2(int wireframe);
GLuint      build_s1_3(int wireframe);
GLuint      build_s1_4(int wireframe);
GLuint      build_s1_5(int wireframe);
GLuint      build_s1_6(int wireframe);
GLuint      build_s1_b(int wireframe);
...
PrintEm(void)
{
	...
	printf("RotX: %d, RotY: %d, Dist: %d.  Targets: X %d, Y %d, D %d.  Visible: %d\n",
...
/* PURIFY 3.0a on SunOS4 reports an unitialized memory read on the next line
 * when using MesaGL 2.2.  This should be fixed in MesaGL 2.3. */
...
InitSproingies(int wfmode, int grnd, int mspr, int screen, int numscreens)
{
	...
	if (si->maxsproingies) {
		si->positions = (void *) calloc(si->maxsproingies, sizeof (struct sPosColor));
	...
	if (!(si->sproingies[0] = build_s1_1(si->wireframe)))
		exit(1);
	if (!(si->sproingies[1] = build_s1_2(si->wireframe)))
		exit(1);
	if (!(si->sproingies[2] = build_s1_3(si->wireframe)))
		exit(1);
	if (!(si->sproingies[3] = build_s1_4(si->wireframe)))
		exit(1);
	if (!(si->sproingies[4] = build_s1_5(si->wireframe)))
		exit(1);
	if (!(si->sproingies[5] = build_s1_6(si->wireframe)))
		exit(1);

	if (!(si->SproingieBoom = build_s1_b(si->wireframe)))
		exit(1);
