#ifndef lint
static char sccsid[] = "@(#)xlock.c	3.0 95/06/28 xlockmore";
#endif
/*-
...
extern void getCryptedUserPasswd();
extern void getCryptedRootPasswd();
extern int checkPasswd();
extern long seconds();
extern void usage();
#ifndef HAS_USLEEP
extern int usleep();
 ...
 */
static void
putText(w, gc, string, bold, left, px, py)
    Window w;         /* which window */
    GC gc;            /* gc */
...
void
set_colormap(scrn, map)
    int scrn;
    Colormap map;
{
    Screen *scr = ScreenOfDisplay(dsp, scrn);
    ...
    char       *argv[];
{
	...
	if (mono || CellsOfScreen(scr) == 2) {
		...
		(void) XAllocColor(dsp, cmap, &xcolor);
