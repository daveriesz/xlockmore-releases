...
float saturate;
{
  ...
  for (i=0 ; i<SWIRLCOLOURS - 1 ; i++)
  {
    ...
    colours[i].r += ((1.0-saturate) * ((float) max-colours[i].r)) ;
    colours[i].g += ((1.0-saturate) * ((float) max-colours[i].g)) ;
    colours[i].b += ((1.0-saturate) * ((float) max-colours[i].b)) ;
  }
}
...
static void    create_colourmap(swirl)
SWIRL_P swirl;
{
...
Window win;
{
  ...
  swirl->rdepth  = win_attr.depth ;
  swirl->visual = win_attr.visual ;

  if (swirl->depth > 16) swirl->depth = 16 ;
  ...
  /* how many knots? */
  swirl->n_knots = random_no((unsigned int) batchcount/2) + batchcount + 1 ;
