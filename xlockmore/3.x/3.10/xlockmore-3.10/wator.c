#ifndef lint
static char sccsid[] = "@(#)wator.c	3.10 96/07/20 xlockmore";
...
init_wator(ModeInfo * mi)
{
	...
	if (wp->pixelmode) {
		wp->ncols = min(wp->width / 2, gridsize);
		wp->nrows = min(wp->height / 2, gridsize);
	} else {
		wp->ncols = min(wp->width / icon_width, gridsize);
		wp->nrows = min(wp->height / icon_height, gridsize);
	}
	if (wp->arr != NULL)
