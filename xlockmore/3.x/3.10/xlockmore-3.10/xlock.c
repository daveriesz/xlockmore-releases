#ifndef lint
static char sccsid[] = "@(#)xlock.c	3.10 96/07/20 xlockmore";
...
#if (!defined(AIXV3) && !defined(__hpux))
...
justDisplay(void)
{
	...
	do {
		...
		if (lockdelay && lockdelay <= seconds() - start_time)
			timetodie = True;
	} while (event.type != ButtonPress &&
#ifdef MOUSE_MOTION
		 event.type != MotionNotify &&
#endif
		 event.type != KeyPress && !timetodie);
	for (screen = 0; screen < screens; screen++)
...
main(int argc, char **argv)
{
	...
	for (screen = 0; screen < screens; screen++) {
		...
		rightcol[screen] = allocPixel(dsp, cmap, right3d, "Red");
		leftcol[screen] = allocPixel(dsp, cmap, left3d, "Green");
