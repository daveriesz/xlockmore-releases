...
#ifndef lint
static char sccsid[] = "@(#)swirl.c	3.10 96/07/20 xlockmore";
...
initialise_swirl(ModeInfo * mi, SWIRL_P swirl)
{
	...
	XQueryColor(display, DefaultColormap(display, MI_SCREEN(mi)),
		    &(swirl->fgcol));
	XQueryColor(display, DefaultColormap(display, MI_SCREEN(mi)),
		    &(swirl->bgcol));
}
 ...
 */
static XVisualInfo *
default_visual_info(ModeInfo * mi, Visual * visual, XVisualInfo ** default_info)
{
	Display    *display = MI_DISPLAY(mi);
	...
	if (visual == NULL)
		visual = DefaultVisual(display, MI_SCREEN(mi));
	...
	*default_info = info_list;
	for (i = 0; i < n; i++, (*default_info)++)
		if ((*default_info)->visual == visual &&
		    (*default_info)->screen == MI_SCREEN(mi)) {
			break;
		}
...
create_colourmap(ModeInfo * mi, SWIRL_P swirl)
{
	...
	list = default_visual_info(mi, swirl->visual, &default_info);

	/* how many colours are there altogether? */
	swirl->colours = MI_NPIXELS(mi);
	...
	if (((swirl->bg == swirl->black) || (swirl->bg == swirl->white)) &&
	...
	swirl->monochrome = (swirl->colours <= 2);
	...
	if (!swirl->fixed_colourmap) {
		...
		if (default_info->class == TrueColor) {
			...
			for (i = 0; i < swirl->colours; i++)
				(void) XAllocColor(display, DefaultColormap(display, MI_SCREEN(mi)),
						   &(swirl->rgb_values[i]));
		}
	}
}
