...
unsigned short ReadIOsb[4] = { 0, 0, 0, 0};
int ReadPending = 0;
int ReadCompleted = 0;
 ...
 */
unsigned short AmdGetChannel(void)
{
	return(so_chan);
}
 ...
 */
unsigned long int AmdInitRecord()
{
	unsigned long int status;
	unsigned long int iosb[2] = { 0, 0 };
 ...
 */
unsigned long int AmdSelect(int s)
{
	unsigned long int status;
...
/*
 *	AST routine for asynchronous reads
 */
AmdASTRead(int (*callback)())
{
	int status;
	status = (*callback)(ReadBuffer, ReadIOsb[1]);
}

/*
 *	Read a buffer of sound data from the device
 */
unsigned long int AmdRead(int (*callback)())
{
	unsigned long int status;
