...
#ifndef lint
static char sccsid[] = "@(#)galaxy.c	3.10 96/07/20 xlockmore";
...
#define MyXDrawPoint(x,y) \
            XFillArc(display,window,gc,x,y,gp->starsize,gp->starsize,0,23040)
#else
#define MyXDrawPoint(x,y) \
...
init_galaxy(ModeInfo * mi)
{
	...
	if (gp->f_galaxies < -MINGALAXIES) {
		free_galaxies(gp);
		gp->f_galaxies = NRAND(-gp->f_galaxies - MINGALAXIES + 1) + MINGALAXIES;
	} else if (gp->f_galaxies < MINGALAXIES)
		gp->f_galaxies = MINGALAXIES;
	if (!gp->galaxies)
		gp->galaxies = (Galaxy *) calloc(gp->f_galaxies, sizeof (Galaxy));
...
draw_galaxy(ModeInfo * mi)
{
	...
	if (gp->init) {
		...
		for (i = 0; i < gp->f_galaxies; ++i)
			gp->galaxies[i].galcol = NRAND(COLORBASE);

		for (i = 0; i < gp->f_galaxies; ++i) {
			gp->galaxies[i].basecolor = gp->galaxies[i].galcol;

			gp->galaxies[i].starscnt = (NRAND(gp->f_stars / 2)) + gp->f_stars / 2;
