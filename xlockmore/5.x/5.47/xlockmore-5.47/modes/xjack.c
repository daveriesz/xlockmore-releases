...
#define font_ascent(f) (font_height(f)*4/5)
static int char_width(XFontStruct *f)
{
        XRectangle mbRect;
...
#define font_ascent(f) ((f==None)?12:f->ascent)
#define char_width(f) ((mode_font->per_char?mode_font->per_char['n' - mode_font->min_char_or_byte2].rbearing:mode_font->min_bounds.rbearing))
#endif
...
init_xjack(ModeInfo * mi)
{
	...
	if (jp->gc == None) {
		...
		jp->ascent = font_ascent(mode_font);
	...
	jp->char_width = char_width(mode_font);
	if (!jp->char_width)
		jp->char_width = 10;
	if (!jp->height)
		jp->height = 15;
...

			{
				...
				do {
					(void) XDrawString (display, window, jp->gc,
							...
							(jp->y * jp->line_height) + jp->vspace + jp->ascent + yshift,
							&c, 1);
		...
		for (i = 0; i < n; i++) {
			...
			while (*n1) {
				(void) XDrawString (display, window, jp->gc,
									...
									(jp->y * jp->line_height) + jp->vspace + jp->ascent,
									n1, 1);
				jp->x++;
				if (jp->x >= jp->columns)
					jp->x = 0, jp->y++;
