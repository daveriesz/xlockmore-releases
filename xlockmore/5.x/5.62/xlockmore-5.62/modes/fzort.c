  ...
  struct pvertex *v0, struct pvertex *v1)
{
	...
	if (y_max <= edge->sy) {
		edge->lines = edge->dxdy = edge->sx = 0;
	} else {
...
make_sphere(int density)
{
	...
	mesh->vtx = (struct vertex *) malloc(mesh->nvtx * sizeof *mesh->vtx);

	/* polygons */

	mesh->npoly = (density + 1)*density;
	poly = mesh->poly = (struct polygon *) malloc(mesh->npoly * sizeof *mesh->poly);
...
make_image(ModeInfo *mi, struct fzort_ctx *fz)
{
	...
	fz->image = create_xshm_image (MI_DISPLAY(mi), MI_VISUAL(mi),
	  depth, format, &fz->shm_info, img_width, img_height);
...
init_fzort_ctx(ModeInfo *mi, fzort_ctx *fz)
{
	...
	if ((fz->pvtx = (struct pvertex *) malloc(fz->mesh->nvtx * sizeof *fz->pvtx)) == NULL) {
		goto failed_3;
	}

	if ((fz->order_in = (struct render_order *) malloc(fz->mesh->npoly * sizeof *fz->order_in))
	  == NULL) {
		goto failed_4;
	}

	if ((fz->order_out = (struct render_order *) malloc(fz->mesh->npoly * sizeof *fz->order_out))
	  == NULL) {
		goto failed_5;
	}
...
draw_fzort(ModeInfo *mi)
{
	...
	put_xshm_image(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi),
	  fz->image,
