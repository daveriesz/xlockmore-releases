 ...
 * Feb-1996: combined with rastering.  Jouk Jansen <joukj AT hrem.nano.tudelft.nl>.
...
ENTRYPOINT ModeSpecOpt puzzle_opts =
...
static puzzlestruct *puzzles = (puzzlestruct *) NULL;
...
static void
free_puzzle_screen(Display * display, puzzlestruct * pp)
{
	if (pp == NULL) {
		return;
	}
...
release_puzzle(ModeInfo * mi)
{
	if (puzzles != NULL) {
		...
		for (screen = 0; screen < MI_NUM_SCREENS(mi); screen++)
			free_puzzle_screen(MI_DISPLAY(mi), &puzzles[screen]);
		free(puzzles);
		puzzles = (puzzlestruct *) NULL;
	}
#ifdef NUMBERED
	if (mode_font != None) {
...
XSCREENSAVER_MODULE ("Puzzle", puzzle)
