...
static int 
middleColumn(int size, int n) {
	return (size - n) * (size - n) - (size - n);
}

static int
middleTlBr(int size, int n) {
	int sum;
	if ((size + n) % 2 == 0)
		sum = ((size + n) / 2) * ((size + n) / 2) + 1 + (n / 2) * 4; 
	 else
		sum = ((size + n) / 2) * ((size + n) / 2) + 2 + ((n - 1) / 2) * 4; 
	if (size % 2 == 1)
		sum += 2;
	return sum;
}
...
static int
middleTrBl(int size, int n) {
	int sum;
	if ((size + n) % 2 == 0)
		sum = ((size + n) / 2 + 1) * ((size + n) / 2 + 1) - 2 - (n / 2) * 4;
	else
		sum = ((size + n) / 2 + 1) * ((size + n) / 2 + 1) - 3 - ((n - 1) / 2) * 4;
	if (size % 2 == 1)
		sum -= 2;
	return sum;
}
...
{
	octstruct *op = &oct[MI_SCREEN(mi)];
	int s0, s1, s2, i, j, k, edge, middle;
	int size = op->size;
	...
	for (middle = 1; middle < 2 * size / 3 - 1; middle++) {
		/* INLINE CENTER FACE */
		int half = (middle - 1) / 2;
		int m1, m2;
		int a, b, c;
		tetra = (middle % 2 == 0);
		s1 = middle + 1;
		s2 = 2 * size - middle - 5 - 2 * half;
		a = middleColumn(size, size - 2 - middle);
		b = middleTrBl(size, size - 2 - middle);
		c = middleTlBr(size, size - 2 - middle);
		m1 = 1;
		m2 = middle + 1;
		if (all || (use && fd.face == 0 && fd.depth == 0)
			...
			drawFacet(mi, a, -1, -1, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 0 && fd.depth == 0)
				|| (use && fd.face == 1 && fd.depth == m1)
			...
			drawFacet(mi, b, -1, -1, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 0 && fd.depth == 0)
				|| (use && fd.face == 3 && fd.depth == m1)
			...
			drawFacet(mi, c, -1, -1, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 1 && fd.depth == 0)
			...
			glPushMatrix();
			glTranslate(s1 * FACET_LENGTH, s2 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, a, -1, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 1 && fd.depth == 0)
				|| (use && fd.face == 2 && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(s1 * FACET_LENGTH, s1 * FACET_LENGTH, s2 * FACET_LENGTH);
			drawFacet(mi, -1, b, -1, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 1 && fd.depth == 0)
				|| (use && fd.face == 0 && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(s2 * FACET_LENGTH, s1 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, c, -1, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 2 && fd.depth == 0)
				|| (use && (fd.face == 1 || fd.face == 3) && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, s2 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, a, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 2 && fd.depth == 0)
				|| (use && fd.face == 3 && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(-s2 * FACET_LENGTH, s1 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, b, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 2 && fd.depth == 0)
				|| (use && fd.face == 1 && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, s1 * FACET_LENGTH, s2 * FACET_LENGTH);
			drawFacet(mi, -1, -1, c, -1, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 3 && fd.depth == 0)
				|| (use && (fd.face == 0 || fd.face == 2) && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, s2 * FACET_LENGTH, -s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, a, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 3 && fd.depth == 0)
				|| (use && fd.face == 0 && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, s1 * FACET_LENGTH, -s2 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, b, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 3 && fd.depth == 0)
				|| (use && fd.face == 2 && fd.depth == m1)
			...
			glPushMatrix();
			glTranslate(-s2 * FACET_LENGTH, s1 * FACET_LENGTH, -s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, c, -1, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 0 && fd.depth == size - 1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, -s2 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, a, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 0 && fd.depth == size - 1)
				|| (use && fd.face == 3 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, -s1 * FACET_LENGTH, s2 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, b, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 0 && fd.depth == size - 1)
				|| (use && fd.face == 1 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(-s2 * FACET_LENGTH, -s1 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, c, -1, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 3 && fd.depth == size - 1)
			...
			glPushMatrix();
			glTranslate(s1 * FACET_LENGTH, -s2 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, a, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 3 && fd.depth == size - 1)
				|| (use && fd.face == 2 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(s2 * FACET_LENGTH, -s1 * FACET_LENGTH, s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, b, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 3 && fd.depth == size - 1)
				|| (use && fd.face == 0 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(s1 * FACET_LENGTH, -s1 * FACET_LENGTH, s2 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, c, -1, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 2 && fd.depth == size - 1)
			...
			glPushMatrix();
			glTranslate(s1 * FACET_LENGTH, -s2 * FACET_LENGTH, -s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, -1, a, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 2 && fd.depth == size - 1)
				|| (use && fd.face == 1 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(s1 * FACET_LENGTH, -s1 * FACET_LENGTH, -s2 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, -1, b, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 2 && fd.depth == size - 1)
				|| (use && fd.face == 3 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(s2 * FACET_LENGTH, -s1 * FACET_LENGTH, -s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, -1, c, -1, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 1 && fd.depth == size - 1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, -s2 * FACET_LENGTH, -s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, -1, -1, a, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 1 && fd.depth == size - 1)
				|| (use && fd.face == 0 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(-s2 * FACET_LENGTH, -s1 * FACET_LENGTH, -s1 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, -1, -1, b, tetra);
			glPopMatrix();
		}
		if (all || (use && fd.face == 1 && fd.depth == size - 1)
				|| (use && fd.face == 2 && fd.depth == size - 1 - m1)
			...
			glPushMatrix();
			glTranslate(-s1 * FACET_LENGTH, -s1 * FACET_LENGTH, -s2 * FACET_LENGTH);
			drawFacet(mi, -1, -1, -1, -1, -1, -1, -1, c, tetra);
			glPopMatrix();
		}
	}
	for (middle = 1; middle < size / 3; middle++) {
		/* INLINE PAST CENTER FACE 3 */
		int half = size / 2;
		int m1, m2;
		int a, b, c;
		tetra = ((size + middle) % 2 == 0);
		s1 = size - 1 - middle;
		/*  6(-3), 7(-3), 8(-5), 9(-4,-5), 10(-4,-6), 11(-6,-7), 12(-6,-7,-9) */
		s2 = 2 * middle + (((size + middle) % 2 == 0) ? 1 : 0);
		a = middleColumn(size, middle);
		b = middleTrBl(size, middle);
		c = middleTlBr(size, middle);
		m1 = half - 1;
		m2 = size - 1 - middle;
		if (all || (use && fd.face == 0 && fd.depth == 0)
				|| (use && (fd.face == 1 || fd.face == 3) && fd.depth == m1)
		...
		Bool use, Bool all)
{
	...
	for (middle = 1; middle < size / 3; middle++) {
		...
		if (all || (use && cd.corner == 2 && cd.depth == m1)
				|| (use && cd.corner == 0 && cd.depth == 2 * size - 2 - m1)
				|| (use && cd.corner == 1 && cd.depth == m2)
				|| (!use && cd.corner == 2 && cd.depth != m1)
				|| (!use && cd.corner == 0 && cd.depth != 2 * size - 2 - m1)
		...
		if (all || (use && cd.corner == 0 && cd.depth == m1)
				|| (use && cd.corner == 1 && cd.depth == 2 * size - 2 - m2)
				|| (use && cd.corner == 2 && cd.depth == 2 * size - 2 - m1)
				|| (!use && cd.corner == 0 && cd.depth != m1)
				|| (!use && cd.corner == 1 && cd.depth != 2 * size - 2 - m2)
