 ...
 *
 * Revision History:
 * 26-Oct-01: Allow wild cards on sound-files. (joukj AT hrem.nano.tudelft.nl)
 */
...
#ifndef SOUNDPATH
#if 0
#define SOUNDPATH "/usr/share/xlock/sounds"
#else
...
#ifdef USE_SOUND
extern void playSound(char *filename, Bool verbose);
#endif
...
#ifdef USE_VMSPLAY
/*-
 * Jouk Jansen <joukj AT hrem.nano.tudelft.nl> contributed this
...
/* Found this snippet from modes/glx/xpm-image.c by way of
   Jamie Zawinski <jwz AT jwz.org> Copyright (c) 1998 */
static Bool
bigendian (void)
{
...
sound_esd_load_sample(char *file)
{
   ...
   origfile = strdup( file ? file : "" );
#ifdef DEBUG
   (void) fprintf(stderr, "sound_esd_load_sample: sample not loaded: loading %s\n", origfile);
#endif
