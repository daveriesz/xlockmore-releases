 ...
 *              that many years:
 *              - Increased number of triangles on polyhedra faces and spheres;
 *              - Made animation smoother by reducing angles and steps granu-
 *                larity per cycle (ants positions and strip rotation velocity)
 *                in addition to reducing the delay between each cycle;
 *              - Adjusted lines thickness for better visibility on newer screen
 *                resolutions like 2K, 4K, 5K and 8K.
 ...
 * 01-Mar-2001: backported from xscreensaver by lassauge AT users.sourceforge.net
...
static argtype vars[] =
{
	{(void *) & solidmoebius, (char *) "solidmoebius", (char *) "Solidmoebius", (char *) DEF_SOLIDMOEBIUS, t_Bool},
	{(void *) & noants, (char *) "noants", (char *) "Noants", (char *) DEF_NOANTS, t_Bool}
...
static Bool
draw_moebius_ant(float *Material, int mono)
{
...
draw_moebius_strip(ModeInfo * mi)
{
	...
	if (!noants) {
		...
		if (!draw_moebius_ant(MaterialYellow, mono))
			return False;
		glPopMatrix();
		...
		glTranslatef(0.28, 0, -0.45);
		if (!draw_moebius_ant(MaterialBlue, mono))
			return False;
		glPopMatrix();
		...
		glTranslatef(0.28, 0, 0.45);
		glRotatef(180, 1, 0, 0);
		if (!draw_moebius_ant(MaterialGreen, mono))
			return False;
		glPopMatrix();
		...
		glTranslatef(0.28, 0, 0.45);
		glRotatef(180, 1, 0, 0);
		if (!draw_moebius_ant(MaterialCyan, mono))
			return False;
		glPopMatrix();
	}
	mp->ant_position += 0.5;
