		...
		for (j = 0; j < SQ_ADAM_LOOPY(evolve); j++)
			for (i = 0; i < SQ_ADAM_LOOPX(evolve); i++) {
			  ...
			  drawcell(mi, start.x + dirx.x * i + diry.x * j - lp->bx,
			   ...
			   (evolve) ? ((lp->clockwise) ? evo_self_reproducing_loop[j][EVO_ADAM_LOOPX - i - 1] : evo_self_reproducing_loop[j][i]) :
			   ((lp->clockwise) ? self_reproducing_loop[j][ADAM_LOOPX - i - 1] : self_reproducing_loop[j][i]));
#endif
			}
	}
}
