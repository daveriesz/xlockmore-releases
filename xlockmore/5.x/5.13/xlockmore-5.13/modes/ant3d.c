		...
		int p1, int p2, int p3, int p4)
{
	...
	if (!ap->wireframe)
		XFillPolygon(display, ap->dbuf, gc, facepts, 4,
	...
	XDrawLines(display, ap->dbuf, gc, facepts, 5, CoordModeOrigin);
}
...
DrawScreen(ModeInfo * mi)
{
	...
	XSetForeground(display, gc, MI_BLACK_PIXEL(mi));
	XFillRectangle(display, ap->dbuf, gc, 0, 0,
		ap->width, ap->height);
	...
	XFlush(display);
	XCopyArea(display, ap->dbuf, MI_WINDOW(mi), gc,
