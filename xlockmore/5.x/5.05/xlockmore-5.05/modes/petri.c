...
static int randblip (ModeInfo * mi , int doit)
{
   petristruct *sp = &petries[MI_SCREEN(mi)];
    ...
    while (n--) 
    {
	int x = random () % sp->arr_width;
	int y = random () % sp->arr_height;
	int c;
	FLOAT s;
	if (b)
	{
