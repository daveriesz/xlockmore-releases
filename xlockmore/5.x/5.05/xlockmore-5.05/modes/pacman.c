...
free_pacman(Display *display, pacmangamestruct *pp)
{
	...
	if (pp->stippledGC != None) {
		(void)XFreeGC(display, pp->stippledGC);
		pp->stippledGC = None;
	}
	if (pp->ghostPixmap != None) {
		(void)XFreePixmap(display, pp->ghostPixmap);
		pp->ghostPixmap = None;
	}
	for (dir = 0; dir < 4; dir++)
		for (mouth = 0; mouth < MAXMOUTH; mouth++)
			if (pp->pacmanPixmap[dir][mouth] != None) {
				(int)XFreePixmap(display, 
	...
	for (ghost = 0; ghost < pp->nghosts; ghost++) {
		...
		(pp->ghosts[ghost].nextcol == pp->pacman.nextcol)) ||
			...
			(void)XSetForeground(display, 
					     pp->stippledGC, 
					     MI_BLACK_PIXEL(mi));
			(void)XFillRectangle(display, window, 
					    pp->stippledGC,
					    pp->ghosts[ghost].cf, 
					    pp->ghosts[ghost].rf, 
					    pp->spritexs, 
...
setwallcolor(ModeInfo * mi) 
{
	...
	if (MI_NPIXELS(mi) > 2)
		(void)XSetForeground(display, pp->stippledGC, 
				MI_PIXEL(mi, BLUE));
	else
		(void)XSetForeground(display, pp->stippledGC, 
				MI_WHITE_PIXEL(mi));
		...
		const unsigned x, const unsigned y)
{
	...
	(void)XSetFillStyle(display, pp->stippledGC, FillSolid);
	(void)XSetLineAttributes(display, pp->stippledGC, pp->wallwidth,
...
draw_pacman_sprite(ModeInfo * mi)
{
...
#ifdef FLASH
		(void)XFillRectangle(display, window, pp->stippledGC,
	...
	(void)XSetTSOrigin(display, pp->stippledGC, 
			   pp->pacman.cf, pp->pacman.rf);
	if (MI_NPIXELS(mi) > 2)
		(void)XSetForeground(display, pp->stippledGC, 
				     MI_PIXEL(mi, YELLOW));
	...
	(void)XSetStipple(display, pp->stippledGC,
		    pp->pacmanPixmap[dir][pp->pacman.mouthstage]);
#ifdef FLASH
	(void)XSetFillStyle(display, pp->stippledGC, FillStippled);
#else
	(void)XSetFillStyle(display, pp->stippledGC, 
			    FillOpaqueStippled);
#endif
	if (pp->xs < 2 || pp->ys < 2)
...
#define SETPOINT(p, xp, yp) p.x = xp; p.y = yp
	...
	(void)XFillRectangle(display, pp->ghostPixmap, bg_gc,
		0, 0, pp->spritexs, pp->spriteys);
	(void)XFillArc(display, pp->ghostPixmap, fg_gc,
		0, 0, pp->spritexs, pp->spriteys, 0, 11520);
	(void)XFillPolygon(display, pp->ghostPixmap, fg_gc,
		points, 9, Nonconvex, CoordModeOrigin);
	(void)XFreeGC(display, bg_gc);
	(void)XFreeGC(display, fg_gc);
