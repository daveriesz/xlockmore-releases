 ...
 *              on VMS so it works for directories. <tschmidt AT micron.com>
 * 29-Oct-2001: Bug fix for sel_image by Jouk Jansen <joukj AT hrem.nano.tudelft.nl>
...
{
	DIR        *dirp;
	struct dirent *new_entry, **namelist_tmp, **namelist_buf;
 ...
 * leaks like this for the C++ modes, and is OK in the C modes. */
	...
	while ((new_entry = readdir(dirp)) != NULL) {
		...
		if (++num_list_tmp >= size_tmp) {
			int oldsize = size_tmp;

			size_tmp = size_tmp + _MEMBL_;
			if ((namelist_buf = (struct dirent **) realloc(
			...
			/* wait to see if error else need to free */
			namelist_tmp = namelist_buf;
		}
...
#define FROM_RESRC   1
#define FROM_FILE    2
#define FROM_FORMATTEDFILE    3
#define FROM_PROGRAM 4
...
#if !defined(WIN32) && !defined(__cplusplus) && !defined(c_plusplus)
				...
				if (!buf[screen * BUFSIZ])
					...
					if (!strcmp(&buf[screen * BUFSIZ], progerr))
						switch (NRAND(14)) {
							...
							case 8:
								(void) strcat(&buf[screen * BUFSIZ],
									      "(Where is your fortune?  This is not good.)");
								break;
							case 9:
								(void) strcat(&buf[screen * BUFSIZ],
							...
							case 10:
								(void) strcat(&buf[screen * BUFSIZ],
									      "(Hey, dude when are you going to fix this?)");
								break;
							case 11:
								(void) strcat(&buf[screen * BUFSIZ],
									      "(Oh, what sad times are these when\none can not get a harmless fortune.)");
								break;
						}
				}
				p = &buf[screen * BUFSIZ];
...
getFontSet(Display * display)
{
	...
	if ((fs = XCreateFontSet(display,
			(messagefontname) ? messagefontname : DEF_MESSAGEFONT,
			&miss, &n_miss, &def)) == NULL) {
		if (messagefontname) {
			(void) fprintf(stderr, "can not find font: %s, using %s...\n",
				messagefontname, DEF_MESSAGEFONT);
