...
#define HACK_INIT init_solitaire
#define HACK_DRAW draw_solitaire
#define solitaire_opts xlockmore_opts
...
inline Colors Card::whichColor() { return ((whichSuit() % MAXCOLORS) ? col_red : col_black);}
...
static solitairestruct *solitaire = (solitairestruct *) NULL;
...
{
	solitairestruct *bp = &solitaire[MI_SCREEN(a_mi)];
...
{
	{(char *) "-trackmouse", (char *) ".solitaire.trackmouse", XrmoptionNoArg, (caddr_t) "on"},
	{(char *) "+trackmouse", (char *) ".solitaire.trackmouse", XrmoptionNoArg, (caddr_t) "off"}
};
...
#ifdef USE_MODULES
ModStruct solitaire_description =
{(char *) "solitaire", (char *) "init_solitaire",
 (char *) "draw_solitaire", (char *) "release_solitaire",
 (char *) "refresh_solitaire", (char *) "init_solitaire",
 (char *) NULL, &solitaire_opts,
...
/*
 *-----------------------------------------------------------------------------
 *    Initialize solitaire.  Called each time the window changes.
 *-----------------------------------------------------------------------------
 */

void
init_solitaire(ModeInfo * mi)
{
	solitairestruct *bp;
...
void
release_solitaire(ModeInfo * mi)
{
	if (solitaire != NULL) {
		for (int screen = 0; screen < MI_NUM_SCREENS(mi); screen++) {
			solitairestruct *bp = &solitaire[screen];
