...
static OptionStruct desc[] =
{
	...
	{(char *) "-neighbors num", (char *) "cubes 6, 8, 14, 18, 20, 26, or rhombic dodecahedron 12"},
...
/* 20S45B4 frequently has unlimited growth, which is not pretty */
#define SOUPSIZE(s)   (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4) ? 4 : ((lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) ? 6 : 10)
...
invplot(int local_neighbors)
{
	...
	case 8: /* corners only */
		return 1;
	case 12: /* edges only, i.e. only odd #'d cells */
		return 2;
...
codeToPatternedRule(int local_neighbors, paramstruct param)
{
	...
	case 18:
		for (i = 0; i < LIFE_18RULES; i++)
			if (param_18rules[i].survival == param.survival &&
					param_18rules[i].birth == param.birth)
				return i;
		return LIFE_18RULES;
	case 20:
		for (i = 0; i < LIFE_20RULES; i++)
			if (param_20rules[i].survival == param.survival &&
					param_20rules[i].birth == param.birth)
				return i;
		return LIFE_20RULES;
	case 26:
		for (i = 0; i < LIFE_26RULES; i++)
			if (param_26rules[i].survival == param.survival &&
					param_26rules[i].birth == param.birth)
				return i;
		return LIFE_26RULES;
	}
	return 0;
...
printRule(int local_neighbors, char * string, paramstruct param, Bool verbose)
{
	...
	if (verbose)
		(void) fprintf(stdout,
			"rule (Survival/Birth %d neighborhood): ",
			local_neighbors);
...
static void 
position_of_neighbor(life3dstruct * lp, int n, unsigned int *col,
  unsigned int *row, unsigned int *stack)
{
	if (lp->neighbors == 6 || lp->neighbors == 14 ||
		...
		case 4:
			*stack = (*stack + 1 == lp->nstacks) ? 0 : *stack + 1;
			break;
		case 5:
			*stack = (!*stack) ? lp->nstacks - 1 : *stack - 1;
			break;
		}
	} else if (lp->neighbors == 8 || lp->neighbors == 12 ||
			lp->neighbors == 20) {
		n += 6;
	}
	...
	} else if (lp->neighbors == 8 || lp->neighbors == 14) {
		n += 12;
	}
	if (lp->neighbors == 8 || lp->neighbors == 14 ||
			lp->neighbors == 20 || lp->neighbors == 26) {
		switch (n) {
		case 18:
			*stack = (*stack + 1 == lp->nstacks) ? 0 : *stack + 1;
...
parseRule(ModeInfo * mi, char * string)
{
	...
	if (foundSurvival && foundBirth)
		return;
	foundSurvival = foundBirth = False;
	...
	if (rule) {
		...
		while (rule[n]) {
			if (rule[n] == 'P' || rule[n] == 'p') {
				...
				l = rule[n] - '0';
				if (l >= 0 && l <= 9 && l <= lp->neighbors) {	/* no 10..26 */
	...
	if (!foundSurvival || !foundBirth ||
			!(lp->input_param.survival || lp->input_param.birth)) {
...
IncrementNbrs3D(life3dstruct * lp, CellList * cell)
{
	...
	for (i = 0; i < lp->neighbors; i++) {
		x = xc = cell->x;
		y = yc = cell->y;
		z = zc = cell->z;
		if (lp->neighbors == 12 && ((x + y + z) & 1) == 0)
			continue;
	/*if (patternsearch &&
		(x == MAXCOLUMNS / 2 - 1 || y == MAXROWS / 2 - 1 || z == MAXSTACKS / 2 - 1))
		continue;*/
		position_of_neighbor(lp, i, &x, &y, &z);
		if (x != xc || y != yc || z != zc)
			if (!ChangeMem(lp,
...
#define ODDASYMRAND 3
#define EVENASYMRAND 4
...
RandomSoup(ModeInfo * mi, int n, int vx, int vy, int vz)
{
	...
	if (yrand == NOSYMRAND) {
		...
		} else if (xrand == ODDASYMRAND) {
			(void) sprintf(lp->nameString, "%s %srandom pattern", "x", "odd asym ");
			for (stack = hs - vz; stack < hs + vz; ++stack)
			for (row = hr - vy; row < hr + vy; ++row)
			for (col = hc; col < hc + vx; ++col) {
		...
		} else if (xrand == EVENASYMRAND) {
			(void) sprintf(lp->nameString, "%s even asym pattern", "x");
			for (stack = hs - vz; stack < hs + vz; ++stack)
			for (row = hr - vy; row < hr + vy; ++row)
			for (col = hc; col < hc + vx; ++col) {
	...
	} else if (yrand == ODDASYMRAND) {
		if (xrand == NOSYMRAND) {
			(void) sprintf(lp->nameString, "%s odd asym pattern", "y");
			for (stack = hs - vz; stack < hs + vz; ++stack)
			for (row = hr; row < hr + vy; ++row)
			for (col = hc - vx; col < hc + vx; ++col) {
	...
	} else if (yrand == EVENASYMRAND) {
		if (xrand == NOSYMRAND) {
			(void) sprintf(lp->nameString, "%s even asym pattern", "y");
			for (stack = hs - vz; stack < hs + vz; ++stack)
			for (row = hr; row < hr + vy; ++row)
			for (col = hc - vx; col < hc + vx; ++col) {
...
GetPattern(ModeInfo * mi, int pattern_rule, int pattern)
{
	...
	if (filePattern) {
		...
		case 12:
			switch (pattern_rule) {
			case LIFE_12S3B3:
				patptr = &patterns_12S3B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S3B3[2 * pattern][0];
#endif
				break;
			case LIFE_12S456B3:
				patptr = &patterns_12S456B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S456B3[2 * pattern][0];
#endif
				break;
			}
			break;
		case 18:
			switch (pattern_rule) {
			case LIFE_18S45B4:
				patptr = &patterns_18S45B4[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_18S45B4[2 * pattern][0];
#endif
				break;
			}
			break;
		case 20:
			switch (pattern_rule) {
			case LIFE_20S45B4:
				patptr = &patterns_20S45B4[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_20S45B4[2 * pattern][0];
#endif
				break;
			}
			break;
		case 26:
			switch (pattern_rule) {
			case LIFE_26S23B5:
				patptr = &patterns_26S23B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S23B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S25B5:
				patptr = &patterns_26S25B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S25B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S27B5:
				patptr = &patterns_26S27B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S27B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S35B5:
				patptr = &patterns_26S35B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S35B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S36B5:
				patptr = &patterns_26S36B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S36B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S37B5:
				patptr = &patterns_26S37B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S37B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S38B5:
				patptr = &patterns_26S38B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S38B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S4B5:
				patptr = &patterns_26S4B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S4B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S45B5:
				patptr = &patterns_26S45B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S45B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S47B5:
				patptr = &patterns_26S47B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S47B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S56B5:
				patptr = &patterns_26S56B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S56B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S567B6:
				patptr = &patterns_26S567B6[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S567B6[2 * pattern][0];
#endif
				break;
			case LIFE_26S57B6:
				patptr = &patterns_26S57B6[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S57B6[2 * pattern][0];
#endif
				break;
			case LIFE_26S58B5:
				patptr = &patterns_26S58B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S58B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S678B5:
				patptr = &patterns_26S678B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S678B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S8B5:
				patptr = &patterns_26S8B5[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S8B5[2 * pattern][0];
#endif
				break;
			case LIFE_26S67B67:
				patptr = &patterns_26S67B67[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S67B67[2 * pattern][0];
#endif
				break;
			}
			break;
...
static void
DrawFace(ModeInfo * mi, int color, XPoint * pts,
...
DrawCube(ModeInfo * mi, CellList * cell)
{
	...
	*/
	if (lp->wireframe) {
		if (dz <= LEN)
			DrawFace(mi, (int) (BLUE & mask), pts, 4, 5, 7, 6);
		else if (dz >= -LEN)
			DrawFace(mi, (int) (BLUE & mask), pts, 0, 1, 3, 2);
		if (dx <= LEN)
			DrawFace(mi, (int) (GREEN & mask), pts, 1, 3, 7, 5);
		else if (dx >= -LEN)
			DrawFace(mi, (int) (GREEN & mask), pts, 0, 2, 6, 4);
		if (dy <= LEN)
			DrawFace(mi, (int) (RED & mask), pts, 2, 3, 7, 6);
		else if (dy >= -LEN)
			DrawFace(mi, (int) (RED & mask), pts, 0, 1, 5, 4);
	}
	if (dz > LEN)
		DrawFace(mi, (int) (BLUE & mask), pts, 4, 5, 7, 6);
	else if (dz < -LEN)
...
DrawRhombicDodecahedron(ModeInfo * mi, CellList * cell)
{
	...
	*/
	if (lp->wireframe) {
		if (dx <= LEN) {
		DrawFace(mi, (int) (GREEN & mask), pts, 2, 5, 0, 3);
		DrawFace(mi, (int) (GREEN & mask), pts, 13, 4, 0, 6);
		DrawFace(mi, (int) (RED & mask), pts, 3, 0, 4, 1);
		DrawFace(mi, (int) (RED & mask), pts, 5, 12, 6, 0);
		} else if (dx >= -LEN) {
		DrawFace(mi, (int) (GREEN & mask), pts, 2, 9, 12, 5);
		DrawFace(mi, (int) (GREEN & mask), pts, 1, 8, 11, 7);
		DrawFace(mi, (int) (RED & mask), pts, 7, 1, 8, 11);
		DrawFace(mi, (int) (RED & mask), pts, 9, 11, 10, 12);
		}
		if (dy <= LEN) {
		DrawFace(mi, (int) (RED & mask), pts, 3, 0, 4, 1);
		DrawFace(mi, (int) (RED & mask), pts, 7, 1, 8, 11);
		DrawFace(mi, (int) (BLUE & mask), pts, 2, 3, 1, 7);
		DrawFace(mi, (int) (BLUE & mask), pts, 13, 8, 1, 4);
		} else if (dy >= -LEN) {
		DrawFace(mi, (int) (RED & mask), pts, 5, 12, 6, 0);
		DrawFace(mi, (int) (RED & mask), pts, 9, 11, 10, 12);
		DrawFace(mi, (int) (BLUE & mask), pts, 2, 9, 12, 5);
		DrawFace(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		}
		if (dz <= LEN) {
		DrawFace(mi, (int) (BLUE & mask), pts, 2, 3, 1, 7);
		DrawFace(mi, (int) (BLUE & mask), pts, 2, 9, 12, 5);
		DrawFace(mi, (int) (GREEN & mask), pts, 2, 7, 11, 9);
		DrawFace(mi, (int) (GREEN & mask), pts, 2, 5, 0, 3);
		} else if (dz >= -LEN) {
		DrawFace(mi, (int) (BLUE & mask), pts, 13, 8, 1, 4);
		DrawFace(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		DrawFace(mi, (int) (GREEN & mask), pts, 13, 4, 0, 6);
		DrawFace(mi, (int) (GREEN & mask), pts, 13, 10, 11, 8);
		}
	}
	sortx = sorty = sortz = 0;
	...
	for (i = 0; i < 3; i++) {
	  ...
	  if (sortz == i) {
		...
		if (dy < 0)
		  ...
		  DrawFace(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		DrawFace(mi, (int) (GREEN & mask), pts, 13, 4, 0, 6);
		DrawFace(mi, (int) (GREEN & mask), pts, 13, 10, 1, 8);
		if (dy < 0)
		  DrawFace(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		else
		  DrawFace(mi, (int) (BLUE & mask), pts, 13, 8, 1, 4);
...
static Bool
orthogonal(life3dstruct * lp) /* or diagonal (across 2 dimensions) */
{
	if (lp->neighbors == 12)
		return (lp->patterned_rule == LIFE_12S456B3);
	else if (lp->neighbors == 18)
		return (lp->patterned_rule == LIFE_18S45B4);
	else if (lp->neighbors == 20)
		return (lp->patterned_rule == LIFE_20S45B4);
	else if (lp->neighbors == 26)
		return ((lp->patterned_rule == LIFE_26S23B5) ||
			(lp->patterned_rule == LIFE_26S25B5) ||
			(lp->patterned_rule == LIFE_26S27B5) ||
			(lp->patterned_rule == LIFE_26S35B5) ||
			(lp->patterned_rule == LIFE_26S36B5) ||
			(lp->patterned_rule == LIFE_26S37B5) ||
			(lp->patterned_rule == LIFE_26S4B5) ||
			(lp->patterned_rule == LIFE_26S47B5) ||
			(lp->patterned_rule == LIFE_26S58B5));
	else
		return False;
}
...
static Bool populateGlider(life3dstruct * lp, char *glider, int offsetSwitch,
	...
	while ((x = *glider++) != 127) {
		...
		SetList(hsp + x * hoff, vsp + y * voff, asp + z * aoff);
	}
}
...
#define V2 (V/2)
	...
	if (!r) {
		...
		if (hsp > c2)
			hoff = -1;
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			/* Must ensure we have only odd cells (x+y+z=odd)
			   for glider, so this must be even */
		    if ((asp + vsp + hsp) & 1)
			hsp--;
		    if (NRAND(3)) {
			populateGlider(lp, glider_12S3B3[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else { /* lower probability of big glider */
			populateGlider(lp, glider_12S3B3[1], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) {
		    /* Must ensure we have only odd cells (x+y+z=odd)
		       for glider, so this must be even */
			if ((asp + vsp + hsp) & 1)
				hsp--;
			populateGlider(lp, glider_12S456B3[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		    if (LRAND() & 1) {
			populateGlider(lp, glider_18S45B4[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else {
			populateGlider(lp, glider_18S45B4[1], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4) {
		    if (LRAND() & 1) {
			populateGlider(lp, glider_20S45B4[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else {
			populateGlider(lp, glider_20S45B4[1], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S23B5) {
			populateGlider(lp, glider_26S23B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S25B5) {
			populateGlider(lp, glider_26S25B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S27B5) {
			populateGlider(lp, glider_26S27B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S35B5) {
			populateGlider(lp, glider_26S35B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S36B5) {
			populateGlider(lp, glider_26S36B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S37B5) {
			populateGlider(lp, glider_26S37B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S38B5) {
			populateGlider(lp, glider_26S38B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S4B5) {
			populateGlider(lp, glider_26S4B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S45B5) {
			populateGlider(lp, glider_26S45B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S47B5) {
			populateGlider(lp, glider_26S47B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S56B5) {
			populateGlider(lp, glider_26S56B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S567B6) {
			populateGlider(lp, glider_26S567B6[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S57B6) {
			populateGlider(lp, glider_26S57B6[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S58B5) {
			populateGlider(lp, glider_26S58B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
			populateGlider(lp, glider_26S678B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S8B5) {
			populateGlider(lp, glider_26S8B5[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (r == 1) {
		if (orthogonal(lp)) {
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			...
			if ((asp + vsp + hsp) & 1)
				asp--;
		    if (NRAND(3)) {
			populateGlider(lp, glider_12S3B3[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else { /* lower probability of big glider */
			populateGlider(lp, glider_12S3B3[1], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) {
		    /* Must ensure we have only odd cells (x+y+z=odd)
			...
			if ((asp + vsp + hsp) & 1)
				asp--;
			populateGlider(lp, glider_12S456B3[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		    if (LRAND() & 1) {
			populateGlider(lp, glider_18S45B4[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else {
			populateGlider(lp, glider_18S45B4[1], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4) {
		    if (LRAND() & 1) {
			populateGlider(lp, glider_20S45B4[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else {
			populateGlider(lp, glider_20S45B4[1], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S23B5) {
			populateGlider(lp, glider_26S23B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S25B5) {
			populateGlider(lp, glider_26S25B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S27B5) {
			populateGlider(lp, glider_26S27B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S35B5) {
			populateGlider(lp, glider_26S35B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S36B5) {
			populateGlider(lp, glider_26S36B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S37B5) {
			populateGlider(lp, glider_26S37B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S38B5) {
			populateGlider(lp, glider_26S38B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S4B5) {
			populateGlider(lp, glider_26S4B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S45B5) {
			populateGlider(lp, glider_26S45B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S47B5) {
			populateGlider(lp, glider_26S47B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S56B5) {
			populateGlider(lp, glider_26S56B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S567B6) {
			populateGlider(lp, glider_26S567B6[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S57B6) {
			populateGlider(lp, glider_26S57B6[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S58B5) {
			populateGlider(lp, glider_26S58B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
			populateGlider(lp, glider_26S678B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S8B5) {
			populateGlider(lp, glider_26S8B5[0], 1,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else {
		if (orthogonal(lp)) {
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			...
			if ((asp + vsp + hsp) & 1)
				vsp--;
		    if (NRAND(3)) {
			populateGlider(lp, glider_12S3B3[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else { /* lower probability of big glider */
			populateGlider(lp, glider_12S3B3[1], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) {
		    /* Must ensure we have only odd cells (x+y+z=odd)
			...
			if ((asp + vsp + hsp) & 1)
				vsp--;
			populateGlider(lp, glider_12S456B3[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		    if (LRAND() & 1) {
			populateGlider(lp, glider_18S45B4[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else {
			populateGlider(lp, glider_18S45B4[1], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4) {
		    if (LRAND() & 1) {
			populateGlider(lp, glider_20S45B4[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		    } else {
			populateGlider(lp, glider_20S45B4[1], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		    }
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S23B5) {
			populateGlider(lp, glider_26S23B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S25B5) {
			populateGlider(lp, glider_26S25B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S27B5) {
			populateGlider(lp, glider_26S27B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S35B5) {
			populateGlider(lp, glider_26S35B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S36B5) {
			populateGlider(lp, glider_26S36B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S37B5) {
			populateGlider(lp, glider_26S37B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S38B5) {
			populateGlider(lp, glider_26S38B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S4B5) {
			populateGlider(lp, glider_26S4B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S45B5) {
			populateGlider(lp, glider_26S45B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S47B5) {
			populateGlider(lp, glider_26S47B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S56B5) {
			populateGlider(lp, glider_26S56B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S567B6) {
			populateGlider(lp, glider_26S567B6[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S57B6) {
			populateGlider(lp, glider_26S57B6[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S58B5) {
			populateGlider(lp, glider_26S58B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
			populateGlider(lp, glider_26S678B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S8B5) {
			populateGlider(lp, glider_26S8B5[0], 2,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	}
	return True;
...
#if 1
		lp->neighbors =
			(NRAND(r12n2 + r18n1 + r20n1 + r26n4) < r12n2) ? 12 :
