...
XClassHint xclasshint={"xlock","xlock"};
...
#if defined( GLOBAL_UNLOCK ) || defined( PAM )
...
getPassword(void)
{
	...
	if (count_failed > 0) {
		...
		if ((cnt = (char *) malloc(strlen((count_failed == 1) ? failed_attempt : failed_attempts) + 16)) != NULL) {
		  ...
		  putText(dsp, Scr[screen].window, Scr[screen].textgc, "\n", False, left, &x, &y);
		  free(cnt);
		  y -= 2 * (font->ascent + font->descent + 2); /* go up */
		}
	}
	timex = x;
	timey = y;
...
/* Get signature file for VMS 7.0 and higher */
	...
	if (planf != NULL) {
		...
		plantext[i] = (char *) NULL;
		(void) fclose(planf);
	} else {
		plantext[0] = (char *) NULL;
	}
	free(buffer);
	buffer = (char *) NULL;
}
...
void PAM_putText( const struct pam_message *msg, struct pam_response *resp, Bool PAM_echokeys )
{
      ...
      ( msg->msg_style == PAM_PROMPT_ECHO_OFF ) )
  {
    ...
    (void) printf( "Setting Up Text To Be Displayed\n" );
#endif
