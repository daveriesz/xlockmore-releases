...
/*-
 * Copyright (c) 1998 by Jouk Jansen <joukj AT hrem.nano.tudelft.nl>
 *
 ...
 *
 * Copyright (c) 1992 - 95    Q. Alex Zhao, azhao AT cc.gatech.edu
 *
 *     All Rights Reserved
 *
...
create_an_image(ModeInfo *mi, trisstruct *tp, int n)
{
  ...
  if (!tp->use3D) {
    ...
    if (!IS_LEFT(n) && !IS_DOWN(n) && !IS_LEFT_DOWN(n))
      for (x=0;x<G+T;x++)
        for(y=tp->ys-G-T;y<tp->ys;y++)
          SETBIT(x,y)
    if (!IS_RIGHT(n) && !IS_UP(n) && !IS_RIGHT_UP(n))
      for (x=tp->ys-G-T;x<tp->ys;x++)
        for(y=0;y<G+T;y++)
          SETBIT(x,y)
    if (!IS_RIGHT(n) && !IS_DOWN(n) && !IS_RIGHT_DOWN(n))
      for (x=tp->ys-G-T;x<tp->ys;x++)
  ...
  {
    if (!IS_LEFT(n) && !IS_UP(n) && !IS_LEFT_UP(n))
      for (x=0;x<tp->ys/2-RR;x++)
        for(y=0;y<tp->ys/2-RR;y++)
          THREEQUARTERSBIT(x,y)
    if (!IS_LEFT(n) && !IS_DOWN(n) && !IS_LEFT_DOWN(n))
      for (x=0;x<tp->ys/2-RR;x++)
        for(y=tp->ys/2+RR;y<tp->ys;y++)
          THREEQUARTERSBIT(x,y)
    if (!IS_RIGHT(n) && !IS_UP(n) && !IS_RIGHT_UP(n))
      for (x=tp->ys/2+RR;x<tp->ys;x++)
        for(y=0;y<tp->ys/2-RR;y++)
          THREEQUARTERSBIT(x,y)
    if (!IS_RIGHT(n) && !IS_DOWN(n) && !IS_RIGHT_DOWN(n))
      for (x=tp->ys/2+RR;x<tp->ys;x++)
