 ...
 * 24-Mar-1998: xlock version David Bagley <bagleyd AT verizon.net>
 * 1997: xscreensaver version Jamie Zawinski <jwz AT jwz.org>
 */
...
ENTRYPOINT ModeSpecOpt goop_opts =
...
static void
free_goop_screen(Display * display, goopstruct * gp)
{
	int         l;
...
release_goop(ModeInfo * mi)
{
	if (goops != NULL) {
		...
		for (screen = 0; screen < MI_NUM_SCREENS(mi); screen++)
			free_goop_screen(MI_DISPLAY(mi), &goops[screen]);
		free(goops);
