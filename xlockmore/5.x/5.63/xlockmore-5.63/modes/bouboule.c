 ...
 * 15-May-1997: jwz AT jwz.org: turned into a standalone program.
 ...
 * 01-Feb-1996: Patched by Jouk Jansen <joukj AT hrem.nano.tudelft.nl> for VMS
 *              Patched by <bagleyd AT verizon.net> for TrueColor displays
...
release_bouboule(ModeInfo * mi)
{
	if (starfield != NULL) {
		...
		for (screen = 0; screen < MI_NUM_SCREENS(mi); screen++)
			free_bouboule_screen(&starfield[screen]);
		free(starfield);
