 ...
 * 1992: xscreensaver version, noseguy (Jamie Zawinski <jwz AT jwz.org>)
 * 1990: X11 version, xnlock (Dan Heller <argv@sun.com>)
 */

/*-
 * xscreensaver, Copyright (c) 1992 Jamie Zawinski <jwz AT jwz.org>
 *
...
#define getFontHeight(f) ((f == None) ? 8 : (f->ascent + f->descent == 0) ? 16 :  f->ascent + f->descent)
static int getTextWidth(char *string)
{
	int width =  XTextWidth(mode_font, string, strlen(string));

	if (width == 0 && strlen(string) > 0)
...
talk(ModeInfo * mi, Bool force_erase)
{
	...
	Window      window = MI_WINDOW(mi);
	nosestruct *np = &noses[MI_SCREEN(mi)];
	int         width = 0, height = 0, Y, Z, total = 0;
	register char *p, *p2;
	...
	for (height = 0; p && height < MAXLINES; height++) {
		...
		*p2 = 0;
		...
		if (mode_font == None && height == 0)
			width = 8;
		if (mode_font != None && (w =
				getTextWidth(args[height])) > width)
			width = w;
	 ...
	 */
	np->s.width = width + 30;
	np->s.height = height * getFontHeight(mode_font) + 30;
	...
	Y = 15 + getFontHeight(mode_font);

	/* now print each string in reverse order (start at bottom of box) */
	for (Z = 0; Z < height; Z++) {
		DrawString(display, window, np->text_fg_gc,
			np->s.x + 15, np->s.y + Y,
			args[Z], strlen(args[Z]));
		Y += getFontHeight(mode_font);
	}
