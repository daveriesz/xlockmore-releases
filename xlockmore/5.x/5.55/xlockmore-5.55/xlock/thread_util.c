...
#	if defined __GLIBC__ && defined _GNU_SOURCE && !defined NDEBUG
...
#	endif
		...
		if(_has_pthread >= 0)
		{
			/* xlockmore: No get_boolean_resource. */
...
#	elif 0 /* TODO: Untested. defined __VMS */

/*
TODO: Add HAVE_PTHREAD to the make.com as well.

TODO: Where does the __NEW_STARLET definition belong? This probably isn't the right spot.

Built with JACK:: on Deathrow.

SYS$GETSYIW is described in
<http://h71000.www7.hp.com/doc/732final/4527/4527pro_005.html?jumpid=reg_r1002_usen_c-001_title_r0001>.

Linking apparently just requires the system default libraries.

Guide to the POSIX Threads Library, Appendix B: Considerations for OpenVMS Systems
...
unsigned hardware_concurrency(void)
{
	...
	ile.ile0.ile3$w_length = sizeof(result);
	ile.ile0.ile3$w_code = SYI$_ACTIVECPU_CNT; /* SYI$_AVAILCPU_CNT is another choice. */
	ile.ile0.ile3$ps_bufaddr = &result;
	ile.ile0.ile3$ps_retlen_addr = &result_size;
	ile.end = 0;

	status = sys$getsyiw(EFN$C_ENF, NULL, NULL, &ile, &iosb, NULL, 0);
	assert(!(status & 1) || !(iosb.iosb$w_status & 1) || result_size == sizeof(result));

	return (status & 1) && (iosb.iosb$w_status & 1) ? result : 1;
}
...
/* xlockmore: Removed unused io_thread code. */
