 ...
 *              unlocking.
 * 23-Dec-1995: Ron Hitchens <ron AT idiom.com> reorganized.
...
extern char error_buf[ERROR_BUF];
...
#endif
/* Hard coding "Password"  is an issue for  non-English locales. */
/* A list of prompts should be obtained at initial phase of authentication. */
#ifdef PAM_PASSWORD_PROMPT
...
static char *crptpass = (char *) NULL;
...
#define SAFELOG_IDENT "/progs/safelog/ident"
	...
	if (!(pw = my_passwd_entry())) {
		...
		(void) sprintf(error_buf, "%s: could not get user shell.\n",
 ...
 */
	...
	if ((pw = getprpwuid(starting_ruid())) == NULL) {
		(void) sprintf(error_buf, "%s: could not get user name.\n",
			(strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
			ProgramName : DEFAULT_NAME);
		error(error_buf);
	}
...
gpasskey(char *pass)
{
	...
	char        *pw, salt[3], key[9];
#if 0
	char       *getpass(const char *);
#endif
	pw = (char *) getpass("  Key: ");
	(void) strncpy(key, pw, 8);
	key[8] = '\0';
	pw = (char *) getpass("Again: ");
	if (strncmp(pw, key, 8) != 0) {
...
{
	FILE       *fp;
	char        xlockrc[MAXPATHLEN+1], *home;
...
checkPasswd(char *buffer)
{
	...
	if (usernet)
		...
		done = ((crptpass = crypt(buffer, userpass)) &&
			!strcmp(userpass, crptpass));
	...
	if (rootnet)
		...
		done = ((crptpass = crypt(buffer, rootpass)) &&
			!strcmp(rootpass, crptpass));
#else /* !DCE_PASSWD */
...
#ifdef SIA
	{
		if ((sia_validate_user(NULL, DEFAULT_NAME, 1, NULL, user, NULL,
...
()), buffer2))
	...
	if (!done) {
		...
		for (pwll = pwllh; pwll->next; pwll = pwll->next)
			if ((crptpass = crypt(buffer, pwll->pw_passwd)) &&
					!strcmp((char *) crptpass, pwll->pw_passwd)) {
#if ( HAVE_SYSLOG_H && defined( USE_SYSLOG ))
...
#ifdef OSF1_ENH_SEC
			(crptpass = dispcrypt(buffer, userpass, user_oldcrypt)) &&
#else
			(crptpass = crypt(buffer, userpass)) &&
#endif
			!strcmp((char *) crptpass, userpass));
		/* userpass is used */
		if (!*userpass && *buffer)
			/*
	...
	if (!done && allowroot &&
#ifdef OSF1_ENH_SEC
			(crptpass = dispcrypt(buffer, rootpass, root_oldcrypt)) &&
#else
			(crptpass = crypt(buffer, rootpass)) &&
#endif
			!strcmp((char *) crptpass, rootpass)) {
		done = True;
		if (!(rootpass != NULL && *buffer))
			/*
			 * root has no password, don't let him in...
			 */
 ...
 * 06-Jul-95: Mods by Heath A. Kehoe <hakehoe AT icaen.uiowa.edu> for
 *            inclusion into xlockmore
 * May-95: Created by Phil O'Connell <philo AT icaen.uiowa.edu>
 */
...
krb_check_password(char *user, char *pass)
{
	...
	option = 0;
	krb5_appdefault_boolean(kcontext, DEFAULT_NAME,
				krb5_princ_realm(context, me), "forwardable",
				option, &option);
	krb5_get_init_creds_opt_set_forwardable(&options, option);

	krb5_appdefault_string(kcontext, DEFAULT_NAME, krb5_princ_realm(context, me),
...
int checkGroupXlockPasswds(char *buffer)
{
	...
	while (grp->gr_mem[i]) {
		unlock = (
#ifdef OSF1_ENH_SEC
			(crptpass = dispcrypt(buffer, &grouppass[i * PASSLENGTH1], user_oldcrypt)) &&
#else
			(crptpass = crypt(buffer, &grouppass[i * PASSLENGTH1])) &&
#endif
			!strcmp((char *) crptpass, &grouppass[i * PASSLENGTH1]));
#if 0
		/* For debugging only! */
...
run_helper(const char *user, const char *pass)
{
  ...
  if (pid == 0) {
    ...
    execlp(PASSWD_HELPER_PROGRAM, PASSWD_HELPER_PROGRAM, DEFAULT_NAME, user, NULL);
