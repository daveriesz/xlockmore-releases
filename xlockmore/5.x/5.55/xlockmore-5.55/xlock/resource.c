 ...
 * 02-Dec-03: Jouk Jansen <joukj AT hrem.nano.tudelft.nl>
 ...
 *            Added DPMS options.
 * 01-Apr-97: Tom Schmidt <tschmidt AT micron.com>
 *            Fixed memory leak.  Made -visual option a hacker mode for now.
 * 20-Mar-97: Tom Schmidt <tschmidt AT micron.com>
 *            Added -visual option.
 *  3-Apr-96: Jouk Jansen <joukj AT hrem.nano.tudelft.nl>
 ...
 *  6-Mar-96: Jouk Jansen <joukj AT hrem.nano.tudelft.nl>
 *            Remote node checking for VMS fixed
 * 20-Dec-95: Ron Hitchens <ron AT idiom.com>
 ...
 * 21-Feb-95: MANY patches from Heath A. Kehoe <hakehoe AT icaen.uiowa.edu>.
...
#else /* !USE_RPLAY */
# if defined ( DEF_PLAY ) || defined ( USE_NAS ) || defined ( USE_ESOUND )
...
# else /* !DEF_PLAY && !USE_NAS && !USE_ESOUND */
#  ifdef USE_VMSPLAY
#   define DEF_LOCKSOUND	"[]thank-you.au"
#   define DEF_INFOSOUND	"[]identify-please.au"
#   define DEF_VALIDSOUND	"[]complete.au"
#   define DEF_INVALIDSOUND	"[]not-programmed.au"
#  endif /* !USE_VMSPLAY */
# endif /* !DEF_PLAY && !USE_NAS && !USE_ESOUND */
#endif /* !USE_RPLAY */
...
static XrmOptionDescRec genTable[] =
{
	...
	{(char *) "-showdate", (char *) ".showdate", XrmoptionNoArg, (caddr_t) "on"},
	{(char *) "+showdate", (char *) ".showdate", XrmoptionNoArg, (caddr_t) "off"},
...
#endif
#ifdef USE_XINERAMA
	{(char *) "-xinerama", (char *) ".xinerama", XrmoptionNoArg, (caddr_t) "on"},
	{(char *) "+xinerama", (char *) ".xinerama", XrmoptionNoArg, (caddr_t) "off"},
#endif
#ifdef USE_SOUND
...
{(char *) "-name resourcename", (char *) "class name to use for resources (default is XLock)"},
...
#endif
	{(char *) "-erasemode erase-modename", (char *) "Erase mode to use (to ddisable set to no_fade)"},
 ...
 {(char *) "-/+allowroot", (char *) "turn on/off allow root password to unlock (off ignored)"},
	...
	{(char *) "-/+showdate", (char *) "turn on/off date in password window"},
...
#endif
#ifdef XINERAMA
	{(char *) "-/+xinerama", (char *) "turn on/off Xinerama"},
#endif
#ifdef USE_SOUND
...
#endif
	{(void *) & showdate, (char *) "showdate", (char *) "ShowDate", (char *) "on", t_Bool},
...
Version(void)
{
	(void) printf("XLock version xlockmore-%s\n", VERSION);
	exit(0);
}
 ...
 * line. It leaks as many bytes as there are characters in the string + 1. */
				...
				if (s == (char *) NULL) {
					(void) sprintf(error_buf,
						"%s: GetResource - could not allocate memory",
 ...
 * the following line. */
	if (!(*displayp = XOpenDisplay((displayname) ? displayname : ""))) {
		int pnl = strlen(ProgramName);
		int dnl = (displayname) ? strlen(displayname) : 0;

		pnl = (pnl < ERROR_BUF - ERROR_LINE) ? pnl :
			strlen(DEFAULT_NAME);
		(void) sprintf(error_buf,
			"%s: unable to open display %s.\n",
			((strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
			ProgramName : DEFAULT_NAME),
			(dnl + pnl < ERROR_BUF - ERROR_LINE) ?
	...
	if (!remote && ((displayname[0] == '_') |
		...
		if (!(status & 1))
			...
			if (strcmp(server_transport, "LOCAL")) {
				...
				pnl = (pnl < ERROR_BUF - ERROR_LINE) ?
					pnl : strlen(DEFAULT_NAME);
				(void) sprintf(error_buf,
					"%s: can not lock %s\n",
					(strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
					ProgramName : DEFAULT_NAME,
...
#endif
#ifdef USE_XINERAMA
	GetResource(*RDB, ProgramName, classname, "xinerama", "Xinerama", t_Bool,
		    (char *) "off", (void *) & xinerama);
#endif
}
...
getResources(Display ** displayp, int argc, char **argv)
{
	...
	XrmParseCommand(&earlyCmdlineDB, earlyCmdlineTable,
			earlyCmdlineEntries, DEFAULT_NAME, &argc, argv);
	GetResource(earlyCmdlineDB, DEFAULT_NAME, classname, "name", "Name",
