 ...
 *
 * xscreensaver code, Copyright (c) 1997 Jamie Zawinski <jwz AT jwz.org>
 *
 ...
 * Revision History:
 *
 * Changes maintained by David Bagley <bagleyd AT verizon.net>
 ...
 * 01-Sep-96: Ron Hitchens <ron AT idiom.com>
 ...
 * 23-Dec-95: Ron Hitchens <ron AT idiom.com>
 ...
 *            <jwz AT jwz.org>, patched the patch (my mistake) for AIXV3
 ...
 *            got courtesy of Jamie Zawinski <jwz AT jwz.org>;
...
#ifdef USE_DPMS
#ifndef MIN_DPMS
#define MIN_DPMS 5		/* 5 second minimum */
#endif
#if 1
#include <X11/Xmd.h>
#include <X11/Xdmcp.h>
...
#define fontDescent (mbRect.height / 5)
#define fontAscent (mbRect.height - fontDescent)
#define planFontHeight (planmbRect.height)
#define planFontDescent (planmbRect.height / 5)
#define planFontAscent (planmbRect.height - planFontDescent)

#else

#define fontHeight (font->ascent + font->descent)
#define fontAscent (font->ascent)
#define fontDescent (font->descent)
#define planFontHeight (planfont->ascent + planfont->descent)
#define planFontAscent (planfont->ascent)
#define planFontDescent (planfont->descent)
...
#if defined( USE_MB ) && defined( __VMS )
int screenOffset = 4;
#else
int screenOffset = 0;
#endif
...
XClassHint xclasshint={(char *) DEFAULT_NAME, (char *) DEFAULT_NAME};
	...
	if (status != GrabSuccess) {
		...
		if (status != GrabSuccess) {
			(void) sprintf(error_buf,
				...
				(strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
				ProgramName : DEFAULT_NAME, status);
			error(error_buf);
		}
	}
...
/*-
 *    Return True or False indicating if the given window has changed
...
#elif defined NL
#include "xlock-msg-nl.h"
#elif defined JA
#include "xlock-msg-ja.h"
#elif defined ZH
#include "xlock-msg-zh_TW.h"
#else
#include "xlock-msg-en.h"
#endif
...
runMainLoop(int maxtime, int iconscreen)
{
	...
	for (;;) {
		...
		if (sleep_time.tv_usec != 0)
			(void) usleep(sleep_time.tv_usec);
		if (sleep_time.tv_sec != 0)
			(void) sleep(sleep_time.tv_sec);
#else
		r = select(fd + 1,
...
modeDescription(ModeInfo * mi)
{
	...
	y = Scr[scrn].iconpos.y - fontHeight + 8;

	XSetForeground(dsp, Scr[scrn].gc, Scr[scrn].bg_pixel);
	XFillRectangle(dsp, Scr[scrn].window, Scr[scrn].gc,
		x, y - fontAscent - 4 + screenOffset ,
		xgwa.width - x, fontHeight + 6 + screenOffset);
...
update_plan(void) /* updates current time in plantext */
{
	if (showdate) {
		...
		if (plantext[0] != NULL) {
			...
			plantext[0][strlen(plantext[0]) - 1] = '\0';
		}
	}
}
...
getPassword(void)
{
	...
	MI_CLEARWINDOWCOLOR(mi, MI_BLACK_PIXEL(mi));
#else
	...
	/* redraw rectangle around icon */
	XDrawRectangle(dsp, Scr[screen].window, Scr[screen].textgc,
		Scr[screen].iconpos.x - 1, Scr[screen].iconpos.y - 1,
		iconwidth + 3, iconheight + 3);
	x = left = Scr[screen].iconpos.x + iconwidth + font->max_bounds.width + 4;
	y = Scr[screen].iconpos.y + fontAscent;
	...
	if (*plantext) {
		...
		for (planp = plantext; *planp; ++planp) {
			int size = strlen(*planp);

			y += planFontHeight + 2;
			/*printf("%s:%d\n", *planp, strlen(*planp));*/
			(void) XDrawString
				(dsp, Scr[screen].window,
	...
	while (!done) {
		...
		if (strlen(pb->chal) > 0) {
			...
			XFillRectangle(dsp, Scr[screen].window, Scr[screen].gc,
				...
				y - fontAscent, XTextWidth(font, chall, strlen(chall)),
				fontHeight +  5);
...
sigcatch(int signum)
{
	...
	(void) sprintf(error_buf,
		...
		(strlen(ProgramName) < ERROR_BUF - 2 * ERROR_LINE) ?
...
#endif
	if (!allowaccess) {
#if defined( SYSV ) || defined( SVR4 ) || ( __VMS_VER >= 70000000 ) || defined( __CYGWIN__ ) || defined( __linux__ )
		sigset_t    oldsigmask;
		sigset_t    newsigmask;
...
#if 0
/* This needs to be freed but this way is invalid */
	if (miss != NULL) {
		int i;
...
main(int argc, char **argv)
{
	...
	pid_t       cmd_pid = 0;
#ifdef USE_XINERAMA
	int         xinerama_evtbase;
	int         xinerama_errbase;
	int         xinerama_nscreens;
...
/* that needs root rights (pam_unix e.g.) */
	...
	if (fontset == NULL) {
		(void) sprintf(error_buf,
			"%s: can not create fonset %s!!!\n",
			(strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
			ProgramName : DEFAULT_NAME,
			(strlen(ProgramName) + strlen(fontsetname) < ERROR_BUF - ERROR_LINE) ?
			fontsetname: "a font");
		error(error_buf);
	}
	...
	if (font == NULL) {
		...
		if (font == NULL) {
			(void) sprintf(error_buf,
				...
				(strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
				ProgramName : DEFAULT_NAME,
				(strlen(ProgramName) + strlen(FALLBACK_FONTNAME) < ERROR_BUF - ERROR_LINE) ?
...
#ifdef USE_XINERAMA
	if (XineramaQueryExtension(dsp, &xinerama_evtbase, &xinerama_errbase))
		xinerama_info = XineramaQueryScreens(dsp, &xinerama_nscreens);
#endif
		...
		for (this_win = 0; this_win < num_wins; this_win++) {
			...
			if (Scr[this_screen].window != None) {
				(void) sprintf(error_buf,
					...
					(strlen(ProgramName) < ERROR_BUF - ERROR_LINE) ?
					ProgramName : DEFAULT_NAME, this_screen);
				error(error_buf);
			}
		...
		if (*plantext) {
			...
			if (debug) {
				Scr[screen].planpos.x = (DisplayWidth(dsp, screen) -
					100 - Scr[screen].planpos.x) / 2;
				Scr[screen].planpos.y = DisplayHeight(dsp, screen) -
					100 - (Scr[screen].planpos.y + 4) * (planFontHeight + 2);
#ifdef USE_XINERAMA
			} else if (xinerama_info) {
				Scr[screen].planpos.x = (xinerama_info[0].width -
					Scr[screen].planpos.x) / 2 + xinerama_info[0].x_org;
				Scr[screen].planpos.y = xinerama_info[0].y_org + xinerama_info[0].height -
...
#define XLOCKWIN_VISUAL CopyFromParent
			...
			XmbSetWMProperties(dsp, Scr[screen].window,
				DEFAULT_NAME, DEFAULT_NAME, NULL, 0, NULL, NULL,
				&xclasshint);
#endif
		}
		if (use3d) {
			...
			if (!install || !XAllocColorCells(dsp, cmap, False, planemasks, 2, pixels,
				...
				/* color overlapping */
				Scr[screen].none_pixel = allocPixel(dsp, cmap,
none3d, (char *)DEF_NONE3D);
				Scr[screen].right_pixel = allocPixel(dsp, cmap,
right3d, (char *)DEF_RIGHT3D);
				Scr[screen].left_pixel = allocPixel(dsp, cmap,
left3d, (char *)DEF_LEFT3D);
				Scr[screen].both_pixel = allocPixel(dsp, cmap,
both3d, (char *)DEF_BOTH3D);
		...
		if (debug || inwindow) {
			...
			xwmh.input = True;
			XSetWMHints(dsp, Scr[screen].window, &xwmh);
		}
		if (debug) {
			Scr[screen].iconpos.x = (DisplayWidth(dsp, screen) - 100 -
#ifdef HAVE_KRB5
				MAX(512, MAX(XTextWidth(font, text_info, strlen(text_info)), XTextWidth(font, text_krbinfo, strlen(text_krbinfo))))
#else /* HAVE_KRB5 */
				MAX(512, XTextWidth(font, text_info, strlen(text_info)))
#endif /* HAVE_KRB5 */
				) / 2;
			Scr[screen].iconpos.y = (DisplayHeight(dsp, screen) - 100) / 6;
#ifdef USE_XINERAMA
		} else if (xinerama_info) {
			Scr[screen].iconpos.x = (xinerama_info[0].width -
#ifdef HAVE_KRB5
					MAX(512, MAX(XTextWidth(font, text_info, strlen(text_info)), XTextWidth(font, text_krbinfo, strlen(text_krbinfo))))
#else /* HAVE_KRB5 */
					MAX(512, XTextWidth(font, text_info, strlen(text_info)))
#endif /* HAVE_KRB5 */
				) / 2 + xinerama_info[0].x_org;
			Scr[screen].iconpos.y = xinerama_info[0].y_org + xinerama_info[0].height / 6;
#endif
		} else {
...
void PAM_putText( const struct pam_message *msg, struct pam_response *resp, Bool PAM_echokeys )
{
  ...
  else
  {
    ...
    XFillRectangle(dsp, Scr[startscreen].window, Scr[startscreen].gc,
		Scr[screen].iconpos.x, y - fontAscent,
...
static int get_random_mode(void)
{
	extern char* xlock95_get_modelist(void);   /* from xlock95.c */
	char *modelist, *p, *modestart;
	int nmodes, randommode, curmode, i;

	/* Get a list of the selected modes */
	modelist = xlock95_get_modelist();

	/* Determine how many modes are selected */
	nmodes = 0;
	p = modelist;
	while (*p) {
		if (*p == ',')
	...
	/* When the user deselected them all, ignore it */
	if (nmodes == 0)
		return NRAND(numprocs);

	/* Get the random mode */
	randommode = NRAND(nmodes);

	/* Get the name of this random mode */
	p = modelist;
	modestart = p;
	curmode = 0;
	...
	/* Get the mode number based upon the name */
	for (i = 0; i < numprocs; i++)
		if (strncmp(LockProcs[i].cmdline_arg,
				modestart, p - modestart) == 0)
			return(i);

	/* Should never come here, just to prevent compile error */
	return NRAND(numprocs);
}
...
unsigned int xlockmore_create(void)
{
	...
	ModeSpecOpt *ms;
	extern TCHAR szSaverName[_MAX_PATH];

    /* seeds random number generation */
	SRAND((unsigned)time(NULL));
/*#define _DEBUG*/

	if (strlen(szSaverName) > 0) {
		randommode = NRAND(numprocs);		/* Start with a random number */
		for (i = 0; i < numprocs; i++) {
			/* did the name on the command line match a savers name? */
			if (stricmp(LockProcs[i].cmdline_arg, szSaverName) == 0)
			{
				randommode = i;
				break;
			}
...
/* This may cause more problems than worth. */
#if 0
#ifndef RANDOMMODE
...
#endif
#ifdef _DEBUG
  xlockmore_win32_text(10, 60, debug_text, 0);
#endif
