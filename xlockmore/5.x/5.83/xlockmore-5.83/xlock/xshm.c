...
/* xlockmore: No progname. */
#include "xlock.h"
#define progname ProgramName
		   ...
		   unsigned int width, unsigned int height)
{
  ...
  Status status;
  XImage *image = 0;
  if (/* xlockmore: skip this */
      /* !get_boolean_resource(dpy, "useSHM", "Boolean") || */
...
put_xshm_image (Display *dpy, Drawable d, GC gc, XImage *image,
                ...
                unsigned int width, unsigned int height,
                XShmSegmentInfo *shm_info, Bool wait)
{
#ifdef HAVE_XSHM_EXTENSION
  assert (shm_info); /* Don't just s/XShmPutImage/put_xshm_image/. */
  if (shm_info->shmid != -1) {
    Bool result = XShmPutImage (dpy, d, gc, image, src_x, src_y,
                                dest_x, dest_y, width, height, wait);
