...
typedef struct trajectory {
  ...
  /* LinkedAction */
  int color;
  int spin, degree_offset, divisions;
...
#define INSERT_AFTER(t, ot, ret)					\
  if ((t = (Trajectory *)calloc(1, sizeof(Trajectory))) == NULL) \
    {free_juggle(sp); return ret;}				\
  (t)->next = (ot)->next;					\
  (t)->prev = (ot);						\
  (ot)->next = (t);						\
  (t)->next->prev = (t)
...
add_throw(jugglestruct *sp, char type, int h, Notation n)
{
  ...
  INSERT_AFTER(t, sp->head->prev, False);
  t->posn = type;
  if (n == ADAM) {
	t->adam = h;
	t->status = ATCH;
...
#define POSITION_BIAS 20 /* larger makes hand movements less likely */
	  ...
	  while (count < MI_CYCLES(mi)) {
		...
		{ /* vary number of balls */
		  ...
		  int change = NRAND(2 + CHANGE_BIAS);
		  switch (change) {
		  case 0:
			new_balls++;
			break;
...
static void
draw_juggle_ball(ModeInfo *mi, unsigned long color, int x, int y, int degree_offset, int divisions)
{
	Display    *dpy = MI_DISPLAY(mi);
	...
	offset = degree_offset << 6; /* multiply by 64 */
	...
	for (traj = sp->head->next; traj != sp->head; traj = traj->next) {
	  ...
	  if (sp->time < traj->start) { /* early */
		...
		if (traj->type == Ball || traj->type == Full) {
		  ...
		  traj->degree_offset = ((traj->degree_offset + 360) + 5 * traj->spin) % 360;
		}
		traj->x = x;
		traj->y = y;
	  } else { /* expired */
