 ...
 *
 * speed is not in input mode
 * DEFAULT for count must be added in app-defaults
 *
 * change-log 0.4pl2 -> 0.5pl0 25/07/2000
...
static void view_3d(XSegment *xseg, xclstruct *dp)
{
  ...
  for(I = 0; I < ENDPOINTS; I++)
    {
      um2( model_data1 [ I * 3 + 0], model_data1 [ I * 3 + 1], model_data1 [ I * 3 + 2],dp);
      BX [I] = dp->Bx;
      BY [I] = dp->By;
    }
...
void init_xcl(ModeInfo * mi)
{
  ...
  MI_CLEARWINDOW(mi);
  /* XFillRectangle(display, MI_WINDOW(mi),dp->erase_gc,0,0,MI_WIDTH(mi),MI_HEIGHT(mi));    */
  
  
  (void) gettimeofday(&(dp->tv1),0);
  dp->time1 = (double)dp->tv1.tv_sec + (double)dp->tv1.tv_usec/(double)1000000;
...
void draw_xcl(ModeInfo * mi)
{
  ...
  if(viewmodel == True)
	...
	dp->Vy = spectator -(int)(sin(dp->az[i]) * cos(dp->el[i]) * line_length);
	dp->Vz = (int)(sin(dp->el[i]) * line_length);

	view_3d(dp->xseg[i],dp);
	XDrawSegments(MI_DISPLAY(mi),MI_WINDOW(mi),dp->erase_gc,dp->xseg_old[i],dp->lines);
	XDrawSegments(MI_DISPLAY(mi),MI_WINDOW(mi),dp->gc[i],dp->xseg[i],dp->lines);
      }
