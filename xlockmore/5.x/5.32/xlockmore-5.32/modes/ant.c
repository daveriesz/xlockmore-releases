...
/*-
 * neighbors of 0 randomizes it for 3, 4, 5, 6, 7, 8, 12 (last 3 are less likely)
 */
	...
	if (ap->neighbors == 6) {
		...
		if (size < -MINSIZE)
			...
			ap->xs = MIN(size, MAX(MINSIZE, MIN(ap->width, ap->height) /
				MINGRIDSIZE));
		ap->ys = ap->xs * 2;
		ap->ncols = (MAX(ap->width / ap->xs, 8) / 4) * 4;
		ap->nrows = (MAX(ap->height / ap->ys, 8)) / 2 * 2;
		ap->xb = (ap->width - ap->xs * ap->ncols) / 2;
		ap->yb = (ap->height - ap->ys * ap->nrows) / 2;
		for (orient = 0; orient < 4; orient++) {
			for (sides = 0; sides < 5; sides++) {
				...
				ap->shape.pentagon[orient][sides].x =
					2 * ap->xs * pentagonUnit[orient][sides].x / 3;
				ap->shape.pentagon[orient][sides].y =
					ap->ys * pentagonUnit[orient][sides].y / 3;
#else
				ap->shape.pentagon[orient][sides].x =
					ap->xs * pentagonUnit[orient][sides].x / 2;
				ap->shape.pentagon[orient][sides].y =
					ap->ys * pentagonUnit[orient][sides].y / 4;
#endif
			}
...
draw_ant(ModeInfo * mi)
{
	...
	for (i = 0; i < ap->n; i++) {
		...
		chg_dir = (2 * ANGLES(ap->neighbors) - status->direction) % ANGLES(ap->neighbors);
		anant->direction = (chg_dir + old_dir) % ANGLES(ap->neighbors);
		if (ap->truchet) {
			int         a = 0, b;

			if (ap->neighbors == 6) {
				if (ap->sharpturn) {
					a = (((ANGLES(ap->neighbors) + anant->direction - old_dir) % ANGLES(ap->neighbors)) == 240);
	/* should be some way of getting rid of the init_dir dependency... */
		...
		old_dir = ((status->direction < ANGLES(ap->neighbors)) ? anant->direction : old_dir);
#ifdef DEBUG
		(void) printf("old_dir %d, col %d, row %d", old_dir, anant->col, anant->row);
#endif
		positionOfNeighbor(ap, old_dir, &(anant->col), &(anant->row));
#ifdef DEBUG
