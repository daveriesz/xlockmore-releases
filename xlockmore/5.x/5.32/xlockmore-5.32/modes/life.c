...
typedef struct {
	...
	int         conway, highlife, daynight, callahan, andreen, bays;
...
parseRule(ModeInfo * mi, char * string)
{
	...
	if (lp->conway || lp->highlife || lp->daynight) {
		lp->neighbors = 8;
		lp->polygon = 4;
...
#if 1
		lp->neighbors = (NRAND(r8n3 + r6n3 + r9n1 + r12n12 + r5n1 + r7n1) < r8n3 + r6n3)
			? ((NRAND(r8n3 + r6n3) < r8n3) ? 8 : 6)
	...
	if (lp->polygon == 3) {
		...
		if (size < -MINSIZE)
			...
			lp->xs = MIN(size, MAX(MINSIZE, MIN(lp->width, lp->height) /
				MINGRIDSIZE));
		lp->ys = lp->xs * 2;
		lp->ncols = (MAX(lp->width / lp->xs, 8) / 4) * 4;
		lp->nrows = (MAX(lp->height / lp->ys, 8)) / 2 * 2;
		lp->xb = (lp->width - lp->xs * lp->ncols) / 2;
		lp->yb = (lp->height - lp->ys * lp->nrows) / 2;
		for (orient = 0; orient < 4; orient++) {
			for (sides = 0; sides < 5; sides++) {
				...
				lp->shape.pentagon[orient][sides].x =
					2 * lp->xs * pentagonUnit[orient][sides].x / 3;
				lp->shape.pentagon[orient][sides].y =
					lp->ys * pentagonUnit[orient][sides].y / 3;
#else
				lp->shape.pentagon[orient][sides].x =
					lp->xs * pentagonUnit[orient][sides].x / 2;
				lp->shape.pentagon[orient][sides].y =
					lp->ys * pentagonUnit[orient][sides].y / 4;
#endif
			}
		}
	} else if (lp->polygon == 6) {
