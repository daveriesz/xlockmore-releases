...
init_demon(ModeInfo * mi)
{
	...
	if (dp->neighbors == 6) {
		...
		for (i = 0; i < 6; i++) {
			if (dp->vertical) {
				dp->shape.hexagon[i].x =
					(dp->xs - 1) * hexagonUnit[i].x;
				dp->shape.hexagon[i].y =
		...
		if (size < -MINSIZE)
			...
			dp->xs = MIN(size, MAX(MINSIZE, MIN(dp->width, dp->height) /
				MINGRIDSIZE));
		dp->ys = dp->xs * 2;
		dp->ncols = (MAX(dp->width / dp->xs, 8) / 4) * 4;
		dp->nrows = (MAX(dp->height / dp->ys, 8)) / 2 * 2;
		dp->xb = (dp->width - dp->xs * dp->ncols) / 2;
		dp->yb = (dp->height - dp->ys * dp->nrows) / 2;
		for (orient = 0; orient < 4; orient++) {
			for (sides = 0; sides < 5; sides++) {
#if 0
				dp->shape.pentagon[orient][sides].x =
					2 * dp->xs * pentagonUnit[orient][sides].x / 3;
				dp->shape.pentagon[orient][sides].y =
					dp->ys * pentagonUnit[orient][sides].y / 3;
#else
				dp->shape.pentagon[orient][sides].x =
					dp->xs * pentagonUnit[orient][sides].x / 2;
				dp->shape.pentagon[orient][sides].y =
					dp->ys * pentagonUnit[orient][sides].y / 4;
#endif
			}
		}
	} else {		/* TRI */
