...
codeToPatternedRule(int local_neighbors, paramstruct param)
{
	...
	case 14:
		for (i = 0; i < LIFE_14RULES; i++)
			if (param_14rules[i].survival == param.survival &&
					param_14rules[i].birth == param.birth)
				return i;
		return LIFE_14RULES;
	case 18:
...
GetPattern(ModeInfo * mi, int pattern_rule, int pattern)
{
	...
	if (filePattern) {
		...
		case 14:
			switch (pattern_rule) {
			case LIFE_14S34B4:
				patptr = &patterns_14S34B4[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_14S34B4[2 * pattern][0];
#endif
				break;
			case LIFE_14S34B46:
				patptr = &patterns_14S34B46[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_14S34B46[2 * pattern][0];
#endif
				break;
			}
			break;
...
DrawRhombicDodecahedron(ModeInfo * mi, CellList * cell)
{
	...
	*/
	if (lp->wireframe) {
		if (dx <= LEN) {
		drawQuad(mi, (int) (GREEN & mask), pts, 2, 5, 0, 3);
		drawQuad(mi, (int) (GREEN & mask), pts, 13, 4, 0, 6);
		drawQuad(mi, (int) (RED & mask), pts, 3, 0, 4, 1);
		drawQuad(mi, (int) (RED & mask), pts, 5, 12, 6, 0);
		} else if (dx >= -LEN) {
		drawQuad(mi, (int) (GREEN & mask), pts, 2, 9, 12, 5);
		drawQuad(mi, (int) (GREEN & mask), pts, 1, 8, 11, 7);
		drawQuad(mi, (int) (RED & mask), pts, 7, 1, 8, 11);
		drawQuad(mi, (int) (RED & mask), pts, 9, 11, 10, 12);
		}
		if (dy <= LEN) {
		drawQuad(mi, (int) (RED & mask), pts, 3, 0, 4, 1);
		drawQuad(mi, (int) (RED & mask), pts, 7, 1, 8, 11);
		drawQuad(mi, (int) (BLUE & mask), pts, 2, 3, 1, 7);
		drawQuad(mi, (int) (BLUE & mask), pts, 13, 8, 1, 4);
		} else if (dy >= -LEN) {
		drawQuad(mi, (int) (RED & mask), pts, 5, 12, 6, 0);
		drawQuad(mi, (int) (RED & mask), pts, 9, 11, 10, 12);
		drawQuad(mi, (int) (BLUE & mask), pts, 2, 9, 12, 5);
		drawQuad(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		}
		if (dz <= LEN) {
		drawQuad(mi, (int) (BLUE & mask), pts, 2, 3, 1, 7);
		drawQuad(mi, (int) (BLUE & mask), pts, 2, 9, 12, 5);
		...
		drawQuad(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		drawQuad(mi, (int) (GREEN & mask), pts, 13, 4, 0, 6);
		drawQuad(mi, (int) (GREEN & mask), pts, 13, 10, 11, 8);
		}
	}
	sortx = sorty = sortz = 0;
	...
	for (i = 0; i < 3; i++) {
	  ...
	  if (sortz == i) {
		...
		if (dy < 0)
		  ...
		  drawQuad(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		drawQuad(mi, (int) (GREEN & mask), pts, 13, 4, 0, 6);
		drawQuad(mi, (int) (GREEN & mask), pts, 13, 10, 1, 8);
		if (dy < 0)
		  drawQuad(mi, (int) (BLUE & mask), pts, 13, 6, 12, 10);
		else
		  drawQuad(mi, (int) (BLUE & mask), pts, 13, 8, 1, 4);
	...
	if (!r) {
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			...
			populateGlider(lp, glider_12S456B3[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 14 && lp->patterned_rule == LIFE_14S34B4) {
		    /* Must ensure we have only z odd cells (z=odd)
		       for glider, so this must be even */
			if (asp & 1)
				asp--;
			populateGlider(lp, glider_14S34B4[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (lp->neighbors == 14 && lp->patterned_rule == LIFE_14S34B46) {
		    /* Must ensure we have only z odd cells (z=odd)
		       for glider, so this must be even */
			if (asp & 1)
				asp--;
			populateGlider(lp, glider_14S34B46[0], 0,
				hsp, vsp, asp, hoff, voff, aoff);
