...
static const struct {
  ...
  int       sharp;                /* 1 in Chance of making a bloody mess */
} ObjectDefs[] = {
  { /* Ball */
	show_ball,
	0,
...
static void
show_figure(ModeInfo * mi, unsigned long color, Bool init, Bool red)
{
  jugglestruct *sp = &juggles[MI_SCREEN(mi)];
  XPoint p[7];
  unsigned int i;
  ...
  */
  ...
  int blood;
  unsigned long redColor = MI_PIXEL(mi, 0);

  if (MI_NPIXELS(mi) <= 2)
     red = False;
  blood = (int)red * (NRAND(4) + 1);
  ...
  XSetForeground(MI_DISPLAY(mi), MI_GC(mi), (blood == 1) ? redColor : color);
  i = 0; /* Body */
  ...
  XSetForeground(MI_DISPLAY(mi), MI_GC(mi), (blood == 2) ? redColor : color);
  i = 0;  /* Legs */
  p[i++] = a[7]; p[i++] = a[0]; p[i++] = a[4]; p[i++] = a[8];
  XDrawLines(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi),
			 p, i, CoordModeOrigin);

  XSetForeground(MI_DISPLAY(mi), MI_GC(mi), (blood == 3) ? redColor : color);
  i = 0;  /* Neck */
  p[i++] = a[5]; p[i++] = a[6];
  XDrawLines(MI_DISPLAY(mi), MI_WINDOW(mi), MI_GC(mi),
			 p, i, CoordModeOrigin);

  XSetForeground(MI_DISPLAY(mi), MI_GC(mi), (blood == 4) ? redColor : color);
  /* Head */
...
change_juggle(ModeInfo * mi)
{
  ...
  show_figure(mi, MI_WHITE_PIXEL(mi), True, False);
...
init_juggle(ModeInfo * mi)
{
  ...
  if (juggles == NULL) { /* First-time initialisation */
	...
	show_figure(mi, MI_WHITE_PIXEL(mi), True, False); /* Draw figure.  Also discovers
...
draw_juggle(ModeInfo * mi)
{
  ...
  if(fabs((sp->cx - cx))*sp->scale >= 2.0 ) {
	show_figure(mi, MI_BLACK_PIXEL(mi), False, False);
	sp->cx = cx;
  }
  ...
  o = sp->objects->next;
  if (MI_NPIXELS(mi) > 2 && ObjectDefs[o->type].sharp != 0 &&
		!NRAND(ObjectDefs[o->type].sharp)) { /* Knife */
	if (LRAND() & 1) {
          show_figure(mi, MI_WHITE_PIXEL(mi), False, True);
	  show_arms(mi, MI_WHITE_PIXEL(mi));
	} else {
          show_figure(mi, MI_WHITE_PIXEL(mi), False, False);
  	  show_arms(mi, MI_PIXEL(mi, 0)); /* Blood Red */
       }
  } else
#endif
  {
