...
static argtype vars[] = {
	...
	{(caddr_t *) &modparam_msg, "msg", "Msg", "", t_String	},
	{(caddr_t *) &modparam_fnt, "fnt", "Fnt", DEFAULT_FNT, t_String	}
};
...
#if NOT_YET
typedef enum {
	EXP_TYPE_0, EXP_TYPE_1, EXP_TYPE_2, EXP_TYPE_3, EXP_TYPE_4, EXP_TYPE_5,
	EXP_TYPE_6, EXP_TYPE_7, EXP_TYPE_8, EXP_TYPE_9, EXP_TYPE_A, EXP_TYPE_B,
	EXP_TYPE_C, EXP_TYPE_D, EXP_TYPE_E, EXP_TYPE_F, EXP_TYPE_G, EXP_TYPE_H,
	EXP_TYPE_I, EXP_TYPE_J, EXP_TYPE_K, EXP_TYPE_L, EXP_TYPE_M, EXP_TYPE_N,
	EXP_TYPE_O, EXP_TYPE_P, EXP_TYPE_Q, EXP_TYPE_R, EXP_TYPE_S, EXP_TYPE_T,
	EXP_TYPE_U, EXP_TYPE_V, EXP_TYPE_W, EXP_TYPE_X, EXP_TYPE_Y, EXP_TYPE_Z,
	EXP_TYPE_LOGO, EXP_TYPE_TEXT
} EXPLOSION_TYPE;
...
typedef struct stage	{
	...
	double v0;	/* initial speed of each spark */
	double delta_t;/* value to be added to the time in each iteration for each spark */
	double phi;	/* angle of the each spark as it leaves the explosion center */
	double m;	/* */
	int generation;/* the generation of each spark */
	int x1, y1;
	int x2, y2;
...
typedef struct missile	{
	int stage;	/* firework stage: ST_PARABOLA, ST_EXPLOSION, ST_DEAD */
	int start_time;
	int color1, color2; /* startcolor, index in colors */
	char msg[256];
...
/*
 * for each screen an instance of FIREWORKS
 * will be allocated.
 */
typedef struct {
	GC		*gc;
...
#if NOT_YET
static PYRO *parse_msg(ModeInfo *mi, char *msg)
{
	int pyrocnt = 0, text_in_buffer = FALSE, i;
	PYRO *pyro = NULL;
	char *rov = msg;
	...
	while(*rov)	{
		switch(*rov)	{
			...
			default:
				...
				if(*rov == ' ' && text_in_buffer == FALSE)	break;
	...
	while(*rov)	{
		switch(*rov)	{
			...
			default:	{
				...
				if(!charcode)	charcode = ((unsigned char)*rov++) & 0xFF;
	...
	for(i=0;i<pyrocnt;++i)	{
#ifndef WIN32
		if(pyro[i].textlen)	pyro[i].bitmap = text2bitmap(mi, pyro[i].text);
#endif
		pyro[i].stat = WAIT;
		pyro[i].wait = i * 40;
...
void init_pyro2(ModeInfo * mi)
{
	...
	time_t t = time(NULL);
	uint32 charcode;
	static int t1lib_init_done = FALSE;
...
#if NOT_YET
	if(pyrocnt == NULL && (pyrocnt = calloc(MI_NUM_SCREENS(mi), sizeof(int))) == NULL)
		return;

	if(pyro == NULL && (pyro = calloc(MI_NUM_SCREENS(mi), sizeof(PYRO))) == NULL)
		return;
#endif /* #if NOT_YET */
...
#undef PYRO_RAMP
	...
	if (!modparam_msg || !*modparam_msg) {
		if (ostext == NULL)
			pyro = parse_msg(mi, DEFAULT_MSG);
		} else
			pyro = parse_msg(mi, ostext);
		}
	} else
		pyro = parse_msg(mi, modparam_msg);
...
void draw_pyro2(ModeInfo * mi)
{
	...
	if(getenv("COLTEST")) {
#if defined(DEBUG)
		fprintf(stderr, "red   = %08X\n", MI_RED_MASK(mi));
		fprintf(stderr, "green = %08X\n", MI_GREEN_MASK(mi));
