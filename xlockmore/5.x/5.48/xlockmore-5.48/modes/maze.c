 ...
 * 04-Feb-1995: patch to slow down maze Heath Kehoe <hakehoe AT icaen.uiowa.edu>
 ...
 * 07-Mar-1993: Good ideas from xscreensaver Jamie Zawinski <jwz AT jwz.org>
...
#define DEF_THICK  "False"
#define DEF_THREED  "False"
...
#endif
  {(char *) "-thick", (char *) ".maze.thick", XrmoptionNoArg, (caddr_t) "on"},
  {(char *) "+thick", (char *) ".maze.thick", XrmoptionNoArg, (caddr_t) "off"},
  {(char *) "-threed", (char *) ".maze.threed", XrmoptionNoArg, (caddr_t) "on"},
  {(char *) "+threed", (char *) ".maze.threed", XrmoptionNoArg, (caddr_t) "off"},
};
...
#endif
  {(void *) & thick, (char *) "thick", (char *) "Thick", (char *) DEF_THICK, t_Bool},
  {(void *) & threed, (char *) "threed", (char *) "ThreeD", (char *) DEF_THREED, t_Bool},
};
...
#endif
  {(char *) "-/+thick", (char *) "turn on/off the thick tracing"},
	...
	switch (dir) {
		case 0:
			XFillRectangle(display, window, gc,
				       mp->xb + 3 * mp->space + mp->xs * i,
				       mp->yb - 3 * mp->space + mp->ys * j,
				       mp->xs + 1 - 6 * mp->space - mp->threed,
				       mp->ys + 1 - mp->threed);
			break;
		case 1:
			XFillRectangle(display, window, gc,
...
set_maze_sizes(ModeInfo * mi)
{
	...
	if (MI_IS_FULLRANDOM(mi)) {
		...
		mp->threed = (randNum == 1) ? 1 : 0;
		mp->space = (randNum == 0) ? 0 : 1;
	} else {
		mp->threed = (threed) ? 1 : 0;
		mp->space = (!threed && thick) ? 0 : 1;
	}
	minsize = mp->space * 4 + 3 + mp->threed;
	if (size < -minsize) {
		free_path(mp);
...
{				/* draw the surrounding wall and start/end squares */
	...
	/* set logo */
	if ((mp->ncols > mp->logo_size_x + 2 + 4 * mp->space) &&
	    (mp->nrows > mp->logo_size_y + 2 + 4 * mp->space)) {
		mp->logo_x = NRAND(mp->ncols - mp->logo_size_x - 2 -
			4 * mp->space) + 1 + 2 * mp->space;
		mp->logo_y = NRAND(mp->nrows - mp->logo_size_y - 2 -
			4 * mp->space) + 1 + 2 * mp->space;
