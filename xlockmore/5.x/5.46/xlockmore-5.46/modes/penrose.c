...
match_rules(fringe_node_c * vertex, rule_match_c * matches, int first_only)
{
	...
	unsigned long vertex_hash = 0, lower_bits_mask = ~(VT_TOTAL_MASK << (VT_BITS * (vertex->n_tiles - 1)));
...
fringe_changes(ModeInfo * mi, fringe_node_c * vertex,
	       ...
	       fringe_node_c ** right, fringe_node_c ** faraway,
	       fringe_node_c ** left)
{
	 ...
	 fringe_node_c * vertex, unsigned side, vertex_type_c vtype)
{
	...
	fringe_node_c
		...
		*faraway = (fringe_node_c *) NULL,
		*node;
	unsigned    fc = fringe_changes(mi, vertex, side, vtype, &right, &faraway, &left);
	...
	if (side == S_LEFT) {
		if (right == NULL)
			if ((right = alloc_vertex(mi, vertex_dir(mi, vertex, S_LEFT) -
				...
				return False;
		if (faraway == NULL)
			if ((faraway = alloc_vertex(mi, vertex_dir(mi, left, S_RIGHT) +
		...
		if (left == NULL)
			if ((left = alloc_vertex(mi, vertex_dir(mi, vertex, S_RIGHT) +
				...
				return False;
		if (faraway == NULL)
			if ((faraway = alloc_vertex(mi, vertex_dir(mi, right, S_LEFT) -
	...
	do {
		if (((fc & FC_NEW_LEFT) && fived_equal(node->fived, left->fived))
		    ...
		    || ((fc & FC_NEW_FAR) && fived_equal(node->fived, faraway->fived))) {
	...
	draw_tile(vertex, right, faraway, left, vtype, mi);

	/* Delete vertices that are no longer on the fringe.  Check the others. */
	if (fc & FC_CUT_THIS) {
		tp->fringe.nodes = faraway;
	...
	if (fc & FC_CUT_FAR)
		...
		add_vtype(faraway, fc & FC_CUT_RIGHT ? S_LEFT : S_RIGHT, ftype);
		check_vertex(mi, faraway, tp);
	}
	if (fc & FC_CUT_LEFT)
		delete_vertex(mi, left, tp);
	else {
...
{
	fringe_node_c *right, *left, *faraway;
	...
	n_good = n;
	for (i = 0; i < n; i++) {
		fc = fringe_changes(mi, vertex, side, vtypes[i], &right, &faraway, &left);
		if (fc & FC_BAG) {
			tp->done = True;
			if (MI_IS_VERBOSE(mi)) {
				(void) fprintf(stderr, "Weirdness in add_random_tile()\n");
		...
		if (faraway) {
			...
			if (!legal_move(faraway, s, VT_FAR(vtypes[i]))) {
				no_good |= (1 << i);
				n_good--;
			}
		}
