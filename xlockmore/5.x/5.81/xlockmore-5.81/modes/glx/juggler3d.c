/* juggle, Copyright (c) 1996-2009 Tim Auckland <tda10.geo AT yahoo.com>
 * and Jamie Zawinski <jwz AT jwz.org>
 *
 ...
 *
 * Revision History
 * 30-Apr-2024: bagleyd added -bbags like his juggling balls
...
#include "visgl.h"
#define ENTRYPOINT
#define MI_COLOR_red    (NRAND(50000)+15536)
#define MI_COLOR_green    (NRAND(50000)+15536)
...
#define DEF_BBAGS "True" /* Use Bean Bags */
...
static XrmOptionDescRec opts[] = {
  ...
  {"-bbags",    ".juggle.bbags",    XrmoptionNoArg,  "on"  },
  {"+bbags",    ".juggle.bbags",    XrmoptionNoArg,  "off" },
...
static argtype vars[] = {
  ...
  { &bbags,    "bbags",    "BBags" ,   DEF_BBAGS,    t_Bool   },
  { &balls,    "balls",    "Balls",    DEF_BALLS,    t_Bool   },
...
ENTRYPOINT ModeSpecOpt juggler3d_opts =
 {countof(opts), opts, countof(vars), vars, desc};

#ifdef USE_MODULES
ModStruct   juggler3d_description =
{"juggler3d", "init_juggler3d", "draw_juggler3d", "release_juggler3d",
 "draw_juggler3d", "change_juggler3d", (char *) NULL, &juggler3d_opts,
 10000, 200, 1000, 1, 64, 1.0, "",
 "Shows a 3D Juggler juggling", 0, NULL};
...
static DrawProc show_bbag, show_ball, show_europeanclub, show_torch, show_knife;
static DrawProc show_ring, show_bball;

typedef enum {BBAG, BALL, CLUB, TORCH, KNIFE, RING, BBALL,
  ...
  for (t = sp->head->next; t != sp->head; t = t->next) {
	if (t->status == ACTION) {
	  if (t->action == THROW) {
		if (t->type == Empty) {
		  /* Create new Object */
		  ADD_ELEMENT(Object_j, t->object, sp->objects);
		  t->object->count = 1;
		  t->object->tracelen = 0;
      ...
      for (i = 0; i <= slices; i++)
        {
          if (stripes == -1) /* octahedron ball */
            glMaterialfv (GL_FRONT, GL_AMBIENT_AND_DIFFUSE,
                    (((j >= stacks / 2 && (i % (slices / 2) < (slices / 4))) ||
                    (j < stacks / 2 && (i % (slices / 2) >= (slices / 4))))
                    ? color1 : color2));
...
static int
show_bbag(ModeInfo *mi, unsigned long color, Trace *s)
{
  int polys = 0;
  jugglestruct *sp = &juggles[MI_SCREEN(mi)];
  ...
  gcolor1[0] = MI_COLOR_red   / 65536.0;
  gcolor1[1] = MI_COLOR_green / 65536.0;
  gcolor1[2] = MI_COLOR_blue  / 65536.0;
...

  {
    ...
    polys += striped_unit_sphere (slices, slices, -1,
...
change_juggler3d(ModeInfo * mi)
{
  ...
  MI_POLYGONCOUNT(mi) += show_figure(mi, True);
}
...
init_juggler3d (ModeInfo * mi)
{
  ...
  if (only && *only && strcmp(only, " ")) {
    bbags = balls = clubs = torches = knives = rings = bballs = False;
    if (!strcasecmp (only, "bbags"))   bbags   = True;
    else if (!strcasecmp (only, "balls"))   balls   = True;
      ...
      (void) fprintf (stderr,
               "Juggle: -only must be one of: bbags, balls, clubs, torches,\n"
               "\t knives, rings, or bballs (not \"%s\")\n", only);
  ...
  if (sp->head == 0) {  /* first time initializing this juggler */
	...
	if(!bbags && !balls && !clubs && !torches && !knives && !rings && !bballs)
...
/*		double angle = traj->angle + traj->spin * (sp->time - traj->start);*/
  ...
  MI_POLYGONCOUNT(mi) += show_figure(mi, False);
  MI_POLYGONCOUNT(mi) += show_arms(mi);

  /* Draw Objects */
  glTranslatef(0, 0, ARMLENGTH);
  for (o = sp->objects->next; o != sp->objects; o = o->next) {
	if(o->active) {
          MI_POLYGONCOUNT(mi) += ObjectDefs[o->type].draw(mi, o->color,
                                                        o->trace->prev);
	  o->active = False;
	}
  }
  ...
  if (future < sp->time + 100 * THROW_CATCH_INTERVAL) {
	refill_juggle(mi);
  }  if (sp->time > 1<<30) { /* Hard Reset before the clock wraps */
	release_juggler3d(mi);
	init_juggler3d(mi);
  }
