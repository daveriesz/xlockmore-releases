...
static void 
position_of_neighbor(life3dstruct * lp, int n, int *col, int *row, int *stack)
{
...
#define V2 (V/2)
	...
	r = NRAND(3);
	if (!r) {
		if ((lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) ||
		    (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) ||
		    (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4)) {
			/* orthagonal */
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			...
			hsp--;
		    if (NRAND(3)) {
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 3 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 2 * aoff);
		    } else { /* lower probability of big glider */
			SetList(hsp + 1 * hoff, vsp + 3 * voff, asp + 3 * aoff);
			SetList(hsp + 1 * hoff, vsp + 4 * voff, asp + 4 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 5 * voff, asp + 4 * aoff);
			SetList(hsp + 2 * hoff, vsp + 4 * voff, asp + 5 * aoff);
			SetList(hsp + 3 * hoff, vsp + 3 * voff, asp + 3 * aoff);
			SetList(hsp + 3 * hoff, vsp + 4 * voff, asp + 4 * aoff);
		    }
			...
			if ((asp + vsp + hsp) & 1)
				hsp--;
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 3 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 3 * aoff);
		} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		    if (LRAND() & 1) {
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 3 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 3 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 3 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 3 * voff, asp + 1 * aoff);
		    } else {
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			...
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 3 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 3 * aoff);
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 3 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 3 * aoff);
		    }
		} else if (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4) {
		    if (LRAND() & 1) {
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 3 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 3 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 1 * aoff);
		    } else {
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			...
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 3 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			...
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 2 * aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S56B5) {
			SetList(hsp + 2 * hoff, vsp + 4 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 4 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 4 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 4 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 3 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 4 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 3 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 3 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 3 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 4 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 4 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 4 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 4 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 4 * aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 3 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 2 * hoff, vsp + 0 * voff, asp + 1 * aoff);
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			...
			SetList(hsp + 3 * hoff, vsp + 3 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 4 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 2 * aoff);
			SetList(hsp + 5 * hoff, vsp + 4 * voff, asp + 2 * aoff);
			SetList(hsp + 4 * hoff, vsp + 5 * voff, asp + 2 * aoff);
			SetList(hsp + 3 * hoff, vsp + 3 * voff, asp + 3 * aoff);
			SetList(hsp + 4 * hoff, vsp + 4 * voff, asp + 3 * aoff);
		    }
		} else if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) {
		    /* Must ensure we have only odd cells (x+y+z=odd)
		       for glider, so this must be even */
			...
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 3 * voff, asp + 2 * aoff);
		} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		    if (LRAND() & 1) {
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			...
			SetList(hsp + 4 * hoff, vsp + 0 * voff, asp + 2 * aoff);
			SetList(hsp + 4 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 4 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 3 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 3 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 4 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 4 * voff, asp + 2 * aoff);
			SetList(hsp + 1 * hoff, vsp + 4 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 4 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 4 * voff, asp + 0 * aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
			...
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 3 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 2 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 2 * aoff);
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
			...
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 3 * aoff);
			SetList(hsp + 4 * hoff, vsp + 1 * voff, asp + 4 * aoff);
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 2 * aoff);
			SetList(hsp + 2 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 2 * voff, asp + 5 * aoff);
			SetList(hsp + 5 * hoff, vsp + 2 * voff, asp + 4 * aoff);
			SetList(hsp + 3 * hoff, vsp + 3 * voff, asp + 3 * aoff);
			SetList(hsp + 4 * hoff, vsp + 3 * voff, asp + 4 * aoff);
		    }
		} else if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) {
		    /* Must ensure we have only odd cells (x+y+z=odd)
		       for glider, so this must be even */
			...
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 3 * hoff, vsp + 2 * voff, asp + 0 * aoff);
		} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		    if (LRAND() & 1) {
			SetList(hsp + 0 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 0 * aoff);
			...
			SetList(hsp + 0 * hoff, vsp + 2 * voff, asp + 4 * aoff);
			SetList(hsp + 0 * hoff, vsp + 1 * voff, asp + 4 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 4 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 4 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 3 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 4 * aoff);
			SetList(hsp + 1 * hoff, vsp + 0 * voff, asp + 3 * aoff);
			SetList(hsp + 3 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 3 * hoff, vsp + 0 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 4 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 4 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 4 * hoff, vsp + 0 * voff, asp + 1 * aoff);
		} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
			...
			SetList(hsp + 2 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 2 * voff, asp + 0 * aoff);
			SetList(hsp + 2 * hoff, vsp + 3 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 1 * aoff);
			SetList(hsp + 1 * hoff, vsp + 1 * voff, asp + 0 * aoff);
			SetList(hsp + 1 * hoff, vsp + 2 * voff, asp + 0 * aoff);
