...
static void
RandomSoup(ModeInfo * mi, int n, int vx, int vy)
{
	lifestruct *lp = &lifes[MI_SCREEN(mi)];
	int row, col;
	int xrand, yrand;
	...
	if (yrand != NOSYMRAND) {
		if (xrand == EVENASYMRAND || xrand == ODDASYMRAND)
			xrand -= 2;
	}
	if (xrand != NOSYMRAND) {
		if (yrand == EVENASYMRAND || yrand == ODDASYMRAND)
			yrand -= 2;
	}
	if (lp->neighbors == 6) {
		if (xrand == EVENSYMRAND)
			xrand = ODDSYMRAND;
		else if (xrand == ODDASYMRAND)
			xrand = EVENASYMRAND;
		if (yrand == EVENSYMRAND)
			yrand = ODDSYMRAND;
	...
	} else if (lp->neighbors == 3 || lp->neighbors == 9 ||
			lp->neighbors == 12) {
		if (xrand == ODDSYMRAND)
			xrand = EVENSYMRAND;
	...
	if (yrand == NOSYMRAND) {
		for (row = halfrows - vy; row < halfrows + vy; ++row) {
			if (xrand == NOSYMRAND) {
				...
				(void) sprintf(lp->nameString, "%s %srandom pattern", (swap) ? "y" : "x", (lp->neighbors == 6) ? "sym " : "odd ");
				for (col = halfcols; col < halfcols + vx;
						++col) {

					if (NRAND(100) < n) {
						SetList(col, row);
						if (lp->neighbors == 6) {
						SetList(2 * halfcols - col - !(row & 1), row);
						} else {
				...
				(void) sprintf(lp->nameString, "%s even random pattern", (swap) ? "y" : "x");
				for (col = halfcols; col < halfcols + vx;
						++col) {

					if (NRAND(100) < n) {
						SetList(col, row);
						SetList(2 * halfcols - col - 1, row);
					}
				}
			} else if (xrand == ODDASYMRAND) {
				(void) sprintf(lp->nameString, "%s %srandom pattern", (swap) ? "y" : "x", (lp->neighbors == 6) ? "asym " : "odd asym ");
				for (col = halfcols; col < halfcols + vx;
						++col) {
				...
				(void) sprintf(lp->nameString, "%s even asym pattern", (swap) ? "y" : "x");
				for (col = halfcols; col < halfcols + vx;
						++col) {
		...
		for (row = halfrows; row < halfrows + vy; ++row) {
			if (xrand == NOSYMRAND) {
				(void) sprintf(lp->nameString, "%s odd random pattern", (swap) ? "x" : "y");
				for (col = halfcols - vx; col < halfcols + vx;
						++col) {
				...
				(void) sprintf(lp->nameString, "x %sand y odd random pattern", (lp->neighbors == 6) ? "sym " : "");
				for (col = halfcols; col < halfcols + vx;
						++col) {
					...
					if (NRAND(100) < n) {
						...
						if (lp->neighbors == 6) {
						SetList(2 * halfcols - col - !(row & 1), row);
						SetList(2 * halfcols - col - !(2 * halfrows - (row & 1)), 2 * halfrows - row);
						} else {
						SetList(2 * halfcols - col, row);
						SetList(2 * halfcols - col, 2 * halfrows - row);
						}
					}
				}
			} else if (xrand == EVENSYMRAND) {
				(void) sprintf(lp->nameString, "%s random pattern", (swap) ? "x odd and y even" : "x even and y odd");
				for (col = halfcols; col < halfcols + vx;
						++col) {
					if (NRAND(100) < n) {
						SetList(col, row);
		...
		for (row = halfrows; row < halfrows + vy; ++row) {
			if (xrand == NOSYMRAND) {
				(void) sprintf(lp->nameString, "%s even random pattern", (swap) ? "x" : "y");
				for (col = halfcols - vx; col < halfcols + vx;
						++col) {
				...
				(void) sprintf(lp->nameString, "%s random pattern", (swap) ? "x even and y odd" : "x odd and y even");
				for (col = halfcols; col < halfcols + vx;
						++col) {
					if (NRAND(100) < n) {
						SetList(col, row);
		...
		for (row = halfrows; row < halfrows + vy; ++row) {
			if (xrand == NOSYMRAND) {
				(void) sprintf(lp->nameString, "%s odd asym pattern", (swap) ? "x" : "y");
				for (col = halfcols - vx; col < halfcols + vx;
						++col) {
		...
		for (row = halfrows; row < halfrows + vy; ++row) {
			if (xrand == NOSYMRAND) {
				(void) sprintf(lp->nameString, "%s even asym pattern", (swap) ? "x" : "y");
				for (col = halfcols - vx; col < halfcols + vx;
						++col) {
		...
		if (xrand == 0) {
			...
			for (row = halfrows - vx; row < halfrows + vx; ++row) {
				for (col = row - halfrows + halfcols; col < halfcols + vx; ++col) {
					if (NRAND(100) < n) {
						SetList(col, row);
						SetList(row + halfcols - halfrows, col + halfrows - halfcols);
					}
				}
			}
		} else if (xrand == 1) {
			...
			for (row = halfrows - vx; row < halfrows + vx; ++row) {
				for (col = row - halfrows + halfcols; col < halfcols + vx; ++col) {
					if (NRAND(100) < n) {
						SetList(2 * halfcols - col, row);
						SetList(halfcols - row + halfrows, col + halfrows - halfcols);
					}
				}
			}
		} else if (xrand == 2) {
			...
			for (row = halfrows - vx; row < halfrows + vx; ++row) {
				for (col = row - halfrows + halfcols; col < halfcols + vx; ++col) {
					if (NRAND(100) < n / 2) {
						...
						SetList(halfcols - row + halfrows, col + halfrows - halfcols);
						SetList(col, 2 * halfrows - row);
						SetList(row + halfcols - halfrows, halfrows - col + halfcols);
					}
				}
			}
		} else if (xrand == 3) {
			(void) strcpy(lp->nameString, "full symmetric random pattern");
			for (row = halfrows; row < halfrows + vx; ++row) {
				for (col = row - halfrows + halfcols; col < halfcols + vx; ++col) {
					if (NRAND(100) < n / 2) {
						SetList(col, row);
						SetList(row + halfcols - halfrows, col + halfrows - halfcols);
						...
						SetList(row + halfcols - halfrows, halfrows - col + halfcols);
						SetList(2 * halfcols - col, 2 * halfrows - row);
						SetList(halfcols - row + halfrows, halfrows - col + halfcols);
					}
				}
			}
		}
...
shooter(ModeInfo * mi)
{
	...
	if (lp->neighbors == 6 && (lp->patterned_rule == LIFE_6S2b34B2a ||
		...
		if (lp->patterned_rule == LIFE_12S12B456) {
			/* glider (c/6, p12) */
			if (sides) {
				SetList(hsp + 2 * hoff, vsp + 0 * voff);
				SetList(hsp + 2 * hoff, vsp + 1 * voff);
			...
			if (sides) {
				...
				SetList(hsp + 3 * hoff, vsp + 6 * voff);
				SetList(hsp + 0 * hoff, vsp + 8 * voff);
				SetList(hsp + 3 * hoff, vsp + 8 * voff);
				SetList(hsp + 0 * hoff, vsp + 9 * voff);
				SetList(hsp + 3 * hoff, vsp + 9 * voff);
		...
		if (sides) { /* lower right */
			if (LRAND() & 1) {
				vsp = NRAND(lp->nrows / 4);
				vsp += (LRAND() & 1) ? 3 * lp->nrows / 4 : 0;
				if (LRAND() & 1) {
					hsp = 0;
					if ((hsp + vsp) & 1)
						hsp++;
				...
				hsp = NRAND(lp->ncols / 4);
				hsp += (LRAND() & 1) ? 3 * lp->ncols / 4 : 0;
				if (LRAND() & 1) {
					vsp = 0;
					if ((hsp + vsp) & 1)
						vsp++;
...
init_life(ModeInfo * mi)
{
	...
	if (MI_IS_FULLRANDOM(mi)) {
		...
		int r12n9 = r12n2 + patterns_12rules[3] + patterns_12rules[4] +
			+ patterns_12rules[5] + patterns_12rules[6] +
			+ patterns_12rules[7] + patterns_12rules[8] +
			+ patterns_12rules[9];
#if 1
		lp->neighbors = (NRAND(r8n3 + r6n3 + r12n9) < r8n3) ? 8 :
			(NRAND(r6n3 + r12n9) < r6n3) ? 6 : 12;
#else
		lp->neighbors = 8;
#endif
		if (lp->neighbors == 6 || lp->neighbors == 12) {
			lp->vertical = LRAND() & 1;
		}
#if 1
		lp->allGliders = True;
