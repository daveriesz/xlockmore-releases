 ...
 * 15-Jan-2003: Alternate life rules from Nathan Thompson's "HighLife"
 *		B36/S23 and "Day and Night" B3678/S34678 with
 *		discoveries by David I. Bell.
 ...
 *              mirror image.
 * 08-Dec-1997: Paul Callahan's B2o/S2m34 rule added.
 ...
 *   B2o/S2m34: Birth of x if 2o,
 *     Survival of x if 2m, 3, or 4 neighbors
 *     Assume symmetry.
 *     (2o, 2m, 2p)
 *           O O            O .            O .
 *     2o:  . x .     2m:  . x O     2p:  . x .
 ...
 *     B2o3o4m/S2om4o (original notation: 234'B/22'4S)
 *     <andreen@msmc.edu>
 *           O O            O O            O .
 *     3o:  . x O     3m:  . x .     3p:  . x O
 *           . .            . O            O .
...
/*-
  ...
  Conway's Life: -neighbors 8 -rule B3/S23 LIFE, CONWAY
  Other things to try:
    ...
    -neighbors 6 -rule B2o/S2m34     <CALLAHAN>
    -neighbors 6 -rule B2o3o4m/S2om4o  <ANDREEN>
...
#define DEF_RULE  "B2o/S2m34"   /* CALLAHAN */
#define DEF_RULE  "B2m3o4m/S2om4o"    /* ANDREEN */
...
static OptionStruct desc[] =
{
	...
	{(char *) "-/+nontot", (char *) "turn on/off nontotalistic rule B3/S2ckan3 (known as Rule 3/2ab3)"},
	{(char *) "-/+conway", (char *) "turn on/off Conway's original Life rule B3/S23"},
	{(char *) "-/+highlife", (char *) "turn on/off Thompson's HighLife rule B36/S23"},
	{(char *) "-/+daynight", (char *) "turn on/off Thompson's Day and Night rule B3678/S34678"},
	{(char *) "-/+callahan", (char *) "turn on/off Callahan's hex rule B2o/S2m34"},
	{(char *) "-/+andreen", (char *) "turn on/off Andreen's hex rule B2o3o4m/S2om4o"},
...
static char hexagonalNontot[] = {'o', 'm', 'p'};
static char mooreNontot[] = {'c', 'e', 'k', 'a', 'i', 'n',
	'y', 'q', 'j', 'r', 't', 'w', 'z', 'f'};
...
static int nontotToIndex(char letter)
{
	switch (letter) { /* take advantage that there is no overlap */
	case 'c':
	case 'o':
		return 0;
	case 'e':
...
static char
getNontot(int neighbors, int offset) {
	if (neighbors == 6 && offset >= 0 && offset < 3) 
		return hexagonalNontot[offset];
	else if (neighbors == 8 && offset >= 0 && offset < 14) 
		return mooreNontot[offset];
	else
		return 'a' + offset; /* do something */
}
		...
		if ((param.birth & (1 << l)) != 0) {
			...
			for (g = 0; g < groupnumber[neighborKind][l - FIRSTGROUP]; g++) {
				if (param.birthGroup[l - FIRSTGROUP] & (1 << g)) {
					...
					(void) sprintf(&(string[i]), "%c",
						getNontot(local_neighbors, g));
					i++;
				}
			}
		}
...
parseRule(ModeInfo * mi, char * string)
{
	...
	if (lp->nontot || lp->conway || lp->highlife || lp->daynight) {
		...
		lp->foundBirth = lp->foundSurvival = !MI_IS_FULLRANDOM(mi);
		if (lp->callahan)
			lifeRule = LIFE_6B2oS2m34;
		else if (lp->andreen)
			lifeRule = LIFE_6B2o3o4mS2om4o;
		else if (lp->bays)
	...
	if (rule) {
		...
		while (rule[n]) {
			if (rule[n] == 'P') {
				...
				/* lower case  is used in groups */
				serving = 'B';
			} else if (rule[n] == 'S' || rule[n] == 'E' || rule[n] == 'L') {
				serving = 'S';
			} else if (rule[n] >= '0' && rule[n] <= '9') {
				int found = 0;
                                if (l >= 0 && l <= 9 && l <= lp->neighbors) /* no 10, 11, 12 */
				    l = rule[n] - '0';
				while (rule[n + 1] >= 'a' && rule[n + 1] <= 'z') {
					n++;
					g = nontotToIndex(rule[n]);
...
GetPattern(ModeInfo * mi, int pattern_rule, int pattern)
{
	...
	if (filePattern) {
		...
		case 6:
			switch (pattern_rule) {
			case LIFE_6B2oS2m34:
				patptr = &patterns_6B2oS2m34[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_6B2oS2m34[2 * pattern][0];
#endif
				break;
			case LIFE_6B2o3o4mS2om4o:
				patptr = &patterns_6B2o3o4mS2om4o[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_6B2o3o4mS2om4o[2 * pattern][0];
#endif
				break;
			case LIFE_6B2S35:
		...
		case 8:
			switch (pattern_rule) {
			case LIFE_8B3S2ckan3:
				patptr = &patterns_8B3S2ckan3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_8B3S2ckan3[2 * pattern][0];
#endif
				break;
			case LIFE_8B3S23:
		...
		case 12:
			...
			case LIFE_12B4S35:
				...
				break;
			case LIFE_12B4S356:
				patptr = &patterns_12B4S356[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12B4S356[2 * pattern][0];
#endif
				break;
			case LIFE_12B46S35:
...
shooter(ModeInfo * mi)
{
	...
	if (lp->neighbors == 6 && (lp->patterned_rule == LIFE_6B2oS2m34 ||
	    lp->patterned_rule == LIFE_6B2o3o4mS2om4o ||
...
#endif
		if (lp->patterned_rule == LIFE_6B2oS2m34) {
			if (diagonal) {
		...
		} else if (lp->patterned_rule == LIFE_6B2o3o4mS2om4o) {
			if (diagonal) {
	...
	} else if (lp->neighbors == 8 && (lp->patterned_rule == LIFE_8B3S2ckan3)) {
		...
		SetList(hsp + 0 * hoff, vsp + 3 * voff);
			...
			lp->patterned_rule == LIFE_12B4S356 ||
		...
		} else if (lp->patterned_rule == LIFE_12B4S356) {
 ...
 */
	...
	while (curr != lp->last[DEAD]) {
		...
		if ((lp->param.birth & (1 << count)) || (count >= FIRSTGROUP &&
				...
				(1 << nontotToIndex(nontotalistic6[gcount]))) != 0) ||
				(lp->neighbors == 8 &&
				(lp->param.birthGroup[count - FIRSTGROUP] &
				(1 << nontotToIndex(nontotalistic8[gcount]))) != 0)))) {
