 ...
 *              <tda10.geo AT yahoo.com>.  Ideas from Peitgen & Saupe's
			...
			*/
			for (row=0; row<sample_step; row++) {
				for (column=0; column<sample_step; column++) {
					...
					r = reps(temp,(double) power,top,0,(interior_t) 0,0.0, zpow, zsin);
...
init_mandelbrot(ModeInfo * mi)
{
	...
	if (MI_IS_FULLRANDOM(mi)) {
		...
		mp->dem = (Bool) (LRAND() & 1);
		mp->interior = (interior_t) NRAND(interior_size);
#if 0
	/* too slow */
	  mp->pow = (NRAND(5) == 0);
	  mp->sin = (NRAND(5) == 0);
...
XSCREENSAVER_MODULE ("Mandelbrot", mandelbrot)
