...
*/
#ifndef WIN32
/* Win32 xlockmore is missing support for ZPixmaps, per-mode config. */
...
rasterize_thread (void *Self_Raw)
{
	...
	for (j=T->y0;j<T->y2;j++) {
		...
		for (k=0;k<A->pool.count;k++) {
			const PIXEL0 *inRow = (PIXEL0 *) ALIGN_HINT(A->threads[k]->accMap[j]);
			for (i=0;i<aw;i++) {
...
init_strange(ModeInfo * mi)
{
	...
	if (useAccumulator) {
		...
		if (MI_NPIXELS(mi) <= 2) {
			...
			A->visualClass = MI_VISUALCLASS(mi);
		}
		...
		if (A->visualClass == TrueColor) {
			...
			A->rMask = MI_RED_MASK(mi);
			A->gMask = MI_GREEN_MASK(mi);
			A->bMask = MI_BLUE_MASK(mi);
