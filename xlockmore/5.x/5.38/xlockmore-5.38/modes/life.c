...
parseRule(ModeInfo * mi, char * string)
{
	...
	if (lp->foundSurvival && lp->foundBirth)
		return;
	lp->foundSurvival = lp->foundBirth = False;
	...
	if (lp->nontot || lp->conway || lp->highlife || lp->daynight) {
		...
		lp->foundSurvival = lp->foundBirth = !MI_IS_FULLRANDOM(mi);
		if (lp->nontot)
	...
	if (rule) {
		...
		while (rule[n]) {
			if (rule[n] == 'P' || rule[n] == 'p') {
				lp->allPatterns = True;
				lp->foundSurvival = lp->foundBirth = True;
				if (MI_IS_VERBOSE(mi))
					(void) fprintf(stdout,
...
init_life(ModeInfo * mi)
{
	...
	lp->generation = 0;
	lp->redrawing = 0;
	lp->foundSurvival = lp->foundBirth = 0;
			...
			for (sides = 0; sides < 3; sides++) {
				if (lp->vertical) {
					...
					lp->shape.triangle[orient][sides].y =
						(lp->ys - 1.6) * triangleUnit[orient][sides].y;
				} else {
					lp->shape.triangle[orient][sides].y =
						(lp->xs - 2) * triangleUnit[orient][sides].x;
					lp->shape.triangle[orient][sides].x =
