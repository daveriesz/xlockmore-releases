	...
	if (filePattern) {
		...
		case 26:
			...
			case LIFE_26S57B6:
				...
				break;
			case LIFE_26S57B67:
				patptr = &patterns_26S57B67[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_26S57B67[2 * pattern][0];
#endif
				break;
			case LIFE_26S58B5:
...
orthogonal(life3dstruct * lp) /* or full diagonal */
{
	if (lp->neighbors == 12)
		...
		return ((lp->patterned_rule == LIFE_26S23B5) ||
			...
			((lp->patterned_rule == LIFE_26S56B5) &&
			(LRAND() & 1)) ||
			((lp->patterned_rule == LIFE_26S57B6) &&
			(NRAND(4) == 0)) ||
			((lp->patterned_rule == LIFE_26S58B5) &&
			(NRAND(4) == 0)) ||
			((lp->patterned_rule == LIFE_26S678B5) &&
			(LRAND() & 1)));
	return False;
}
		...
		int hsp, int vsp, int asp, int hoff, int voff, int aoff)
{
	if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S3B3) {
		if (NRAND(3)) {
			(void) populateGlider(lp, glider_12S3B3[0], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		} else if (NRAND(3)) {
			(void) populateGlider(lp, glider_12S3B3[1], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		} else { /* lower probability of big glider */
			(void) populateGlider(lp, glider_12S3B3[2], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S456B3) {
		(void) populateGlider(lp, glider_12S456B3[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 14 && lp->patterned_rule == LIFE_14S34B4) {
		(void) populateGlider(lp, glider_14S34B4[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 14 && lp->patterned_rule == LIFE_14S34B46) {
		(void) populateGlider(lp, glider_14S34B46[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 18 && lp->patterned_rule == LIFE_18S45B4) {
		if (orth) {
			if (LRAND() & 1) {
				(void) populateGlider(lp, glider_18S45B4[0], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			} else {
				(void) populateGlider(lp, glider_18S45B4[1], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			}
		} else {
			(void) populateGlider(lp, glider_18S45B4[2], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 20 && lp->patterned_rule == LIFE_20S45B4) {
		if (LRAND() & 1) {
			(void) populateGlider(lp, glider_20S45B4[0], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		} else {
			(void) populateGlider(lp, glider_20S45B4[1], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S23B5) {
		(void) populateGlider(lp, glider_26S23B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S25B5) {
		(void) populateGlider(lp, glider_26S25B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S27B5) {
		(void) populateGlider(lp, glider_26S27B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S35B5) {
		(void) populateGlider(lp, glider_26S35B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S36B5) {
		(void) populateGlider(lp, glider_26S36B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S37B5) {
		(void) populateGlider(lp, glider_26S37B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S38B5) {
		if (LRAND() & 1) {
			(void) populateGlider(lp, glider_26S38B5[0], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		} else {
			(void) populateGlider(lp, glider_26S38B5[1], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S4B5) {
		(void) populateGlider(lp, glider_26S4B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S45B5) {
		(void) populateGlider(lp, glider_26S45B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S47B5) {
		(void) populateGlider(lp, glider_26S47B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S56B5) {
		if (orth) {
			if (LRAND() & 1) {
				(void) populateGlider(lp, glider_26S56B5[0], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			} else {
				(void) populateGlider(lp, glider_26S56B5[1], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			}
		} else {
			(void) populateGlider(lp, glider_26S56B5[2], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S567B6) {
		(void) populateGlider(lp, glider_26S567B6[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S57B6) {
		if (orth) {
			if (LRAND() & 1) {
				(void) populateGlider(lp, glider_26S57B6[1], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			} else {
				(void) populateGlider(lp, glider_26S57B6[2], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			}
		} else {
			(void) populateGlider(lp, glider_26S57B6[0], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S57B67) {
		(void) populateGlider(lp, glider_26S57B67[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S58B5) {
		if (orth)
			(void) populateGlider(lp, glider_26S58B5[4], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		else if (NRAND(3)) {
			if (LRAND() & 1)
				(void) populateGlider(lp, glider_26S58B5[0], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			else if (LRAND() & 1)
				(void) populateGlider(lp, glider_26S58B5[1], dim,
					hsp, vsp, asp, hoff, voff, aoff);
			else
				(void) populateGlider(lp, glider_26S58B5[2], dim,
					hsp, vsp, asp, hoff, voff, aoff);
		} else
			(void) populateGlider(lp, glider_26S58B5[3], dim,
				hsp, vsp, asp, hoff, voff, aoff);
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S678B5) {
		if (orth) {
			(void) populateGlider(lp, glider_26S678B5[1], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		} else {
			(void) populateGlider(lp, glider_26S678B5[0], dim,
				hsp, vsp, asp, hoff, voff, aoff);
		}
	} else if (lp->neighbors == 26 && lp->patterned_rule == LIFE_26S8B5) {
		(void) populateGlider(lp, glider_26S8B5[0], dim,
			hsp, vsp, asp, hoff, voff, aoff);
	}
}
