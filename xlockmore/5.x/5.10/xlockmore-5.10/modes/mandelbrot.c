...
static argtype vars[] =
{
  ...
  {(caddr_t *) & pow_p, (char *) "pow", (char *) "Pow", (char *) DEF_POW, t_Bool},
  {(caddr_t *) & sin_p, (char *) "sin", (char *) "Sin", (char *) DEF_SIN, t_Bool},
  {(caddr_t *) & cycle_p, (char *) "cycle", (char *) "Cycle", (char *) DEF_CYCLE
, t_Bool}
};
 ...
 */
static int
reps(complex c, double p, int r, Bool binary, double demrange, Bool zpow, Bool zsin)
{
	int         rep;
	int         escaped = 0;
	complex     t;
			...
			*/
			for (row=0; row<sample_step; row++) {
				for (column=0; column<sample_step; column++) {
					...
					r = reps(temp,(double) power,top,0,0.0, zpow, zsin);
  ...
  if (mp->cycle_p) {
	...
	*/
	...
	for (h = 0; h < mp->screen_height; h++) {
		...
		result = reps(c, mp->power, mp->reptop, mp->binary, demrange, mp->pow, mp->sin);
