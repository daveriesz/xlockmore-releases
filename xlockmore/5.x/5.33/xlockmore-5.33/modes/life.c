...
static OptionStruct desc[] =
{
	...
	{(char *) "-/+nontot", (char *) "turn on/off nontotalistic rule S2a2b2c2d3/B3 (known as Rule 3/2ab3)"},
	{(char *) "-/+conway", (char *) "turn on/off Conway's original Life rule S23/B3"},
	{(char *) "-/+highlife", (char *) "turn on/off Bell's HighLife rule S23/B36"},
	{(char *) "-/+daynight", (char *) "turn on/off Thompson's Day and Night rule S34678/B3678"},
	{(char *) "-/+callahan", (char *) "turn on/off Callahan's hex rule S2b34/B2a"},
	{(char *) "-/+andreen", (char *) "turn on/off Andreen's hex rule S2a2b4a/B2a3a4b"},
	{(char *) "-/+bays", (char *) "turn on/off Bays' hex rule S35/B2"},
	{(char *) "-/+trilife", (char *) "turn on/off Bays' tri rule S34/B45"},
	{(char *) "-/+trilife1", (char *) "turn on/off Bays' tri rule S45/B456"},
	{(char *) "-/+trilife2", (char *) "turn on/off Bays' tri rule S23/B45"},
	{(char *) "-/+pent", (char *) "turn on/off Bagley's pentagon rule S24/B2"},
	{(char *) "-/+pent2", (char *) "turn on/off Bays' pentagon rule S23/B346"},
};
...
codeToPatternedRule(int local_neighbors, paramstruct param)
{
	...
	case 5:
		for (i = 0; i < LIFE_5RULES; i++)
			if (param_5rules[i].survival == param.survival &&
			    param_5rules[i].birth == param.birth) {
				for (g = 0; g < maxgroups[neighborKind]; g++) {
					if (param_5rules[i].survivalGroup[g] !=
					    param.survivalGroup[g] ||
					    param_5rules[i].birthGroup[g] !=
					    param.birthGroup[g]) {
						break;
					}
				}
	...
	case 6:
		for (i = 0; i < LIFE_6RULES; i++)
			if (param_6rules[i].survival == param.survival &&
				...
				for (g = 0; g < maxgroups[neighborKind]; g++) {
					if (param_6rules[i].survivalGroup[g] !=
					    param.survivalGroup[g] ||
					    param_6rules[i].birthGroup[g] !=
					    param.birthGroup[g]) {
						break;
					}
				}
	...
	case 7:
		for (i = 0; i < LIFE_7RULES; i++)
			if (param_7rules[i].survival == param.survival &&
			    param_7rules[i].birth == param.birth) {
				for (g = 0; g < maxgroups[neighborKind]; g++) {
					if (param_7rules[i].survivalGroup[g] !=
					    param.survivalGroup[g] ||
					    param_7rules[i].birthGroup[g] !=
					    param.birthGroup[g]) {
						break;
					}
				}
	...
	case 8:
		for (i = 0; i < LIFE_8RULES; i++)
			if (param_8rules[i].survival == param.survival &&
				...
				for (g = 0; g < maxgroups[neighborKind]; g++) {
					if (param_8rules[i].survivalGroup[g] !=
					    param.survivalGroup[g] ||
					    param_8rules[i].birthGroup[g] !=
					    param.birthGroup[g]) {
						break;
					}
				}
	...
	case 9:
		for (i = 0; i < LIFE_9RULES; i++)
			if (param_9rules[i].survival == param.survival &&
			    param_9rules[i].birth == param.birth) {
				for (g = 0; g < maxgroups[neighborKind]; g++) {
					if (param_9rules[i].survivalGroup[g] !=
					    param.survivalGroup[g] ||
					    param_9rules[i].birthGroup[g] !=
					    param.birthGroup[g]) {
						break;
					}
				}
	...
	case 12:
		for (i = 0; i < LIFE_12RULES; i++)
			if (param_12rules[i].survival == param.survival &&
				...
				for (g = 0; g < maxgroups[neighborKind]; g++) {
					if (param_12rules[i].survivalGroup[g] !=
					    param.survivalGroup[g] ||
					    param_12rules[i].birthGroup[g] !=
					    param.birthGroup[g]) {
						break;
					}
				}
...
printRule(int local_neighbors, char * string, paramstruct param, Bool verbose)
{
	...
	for (l = 0; l < maxgroups[neighborKind]; l++) {
		if (param.survivalGroup[l] || param.birthGroup[l]) {
			found = True;
			break;
		}
	}
	if (verbose) {
		(void) fprintf(stdout, "%s\nbinary rule%s: Survival 0x%X, Birth 0x%X\n",
		...
		if (found) {
			for (l = 0; l < maxgroups[neighborKind]; l++) {
				(void) fprintf(stdout,
				       ...
				       l + FIRSTGROUP, param.survivalGroup[l], param.birthGroup[l]);
			}
		}
	}
}
...
parseRule(ModeInfo * mi, char * string)
{
	...
	if (lp->nontot || lp->conway || lp->highlife || lp->daynight) {
		...
		foundSurvival = foundBirth = !MI_IS_FULLRANDOM(mi);
		if (lp->nontot)
			lifeRule = LIFE_8S2a2b2c2d3B3;
		else if (lp->conway)
			lifeRule = LIFE_8S23B3;
		else if (lp->highlife)
			lifeRule = LIFE_8S23B36;
		else if (lp->daynight)
			lifeRule = LIFE_8S34678B3678;
		...
		foundSurvival = foundBirth = !MI_IS_FULLRANDOM(mi);
		if (lp->callahan)
			lifeRule = LIFE_6S2b34B2a;
		else if (lp->andreen)
			lifeRule = LIFE_6S2a2b4aB2a3a4b;
		else if (lp->bays)
			lifeRule = LIFE_6S35B2;
		...
		foundSurvival = foundBirth = !MI_IS_FULLRANDOM(mi);
		if (lp->trilife)
			lifeRule = LIFE_12S34B45;
		else if (lp->trilife1)
			lifeRule = LIFE_12S45B456;
		else if (lp->trilife2)
			lifeRule = LIFE_12S23B45;
	...
	if (rule) {
		...
		while (rule[n]) {
			if (rule[n] == 'P' || rule[n] == 'p') {
				...
				serving = 'S';
			} else if (rule[n] == 'B') { /* lower case is used in groups */
				serving = 'B';
			} else {
				l = rule[n] - '0';
	...
	if (lp->polygon == 3) {
		...
		if (lp->xs <= 3 || lp->ys <= 3)
			...
			if (lp->vertical) {
				...
				if (orient)
					lp->shape.triangle[orient][0].y += (lp->xs / 2 - 1);
				else
					lp->shape.triangle[orient][0].y -= (lp->xs / 2 - 1);
			}
...
RandomSoup(ModeInfo * mi, int symmetryIndex, int percent, int fieldX, int fieldY)
{
	...
	case 4: /* Asym X Random */
		...
		(void) strcpy(lp->nameString, "Asym X Random");
		break;
	case 5: /* Asym Y Random */
		for (row = halfrows; row < halfrows + vy; ++row) {
			for (col = halfcols - vx; col < halfcols + vx; ++col) {
				if (NRAND(100) < percent) {
					if (lp->polygon != 5) {
		...
		(void) strcpy(lp->nameString, "Asym Y Random");
		break;
	case 6: /* Half Asym Random */
		...
		(void) strcpy(lp->nameString, "Half Asym Random");
		break;
	...
	case 8: /* Full Asym Random */
		...
		(void) strcpy(lp->nameString, "Full Asym Random");
		break;
...
GetPattern(ModeInfo * mi, int pattern_rule, int pattern)
{
	...
	if (filePattern) {
		...
		case 5:
			switch (pattern_rule) {
			case LIFE_5S24B2:
				patptr = &patterns_5S24B2[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_5S24B2[2 * pattern][0];
#endif
			break;
			case LIFE_5S2B24:
				patptr = &patterns_5S2B24[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_5S2B24[2 * pattern][0];
#endif
			}
			break;
		case 6:
			...
			case LIFE_6S2a2b4aB2a3a4b:
				...
				break;
			case LIFE_6S35B2:
				patptr = &patterns_6S35B2[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_6S35B2[2 * pattern][0];
#endif
				break;
			case LIFE_6S3B245:
				patptr = &patterns_6S3B245[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_6S3B245[2 * pattern][0];
#endif
				break;
			case LIFE_6S34B2:
				patptr = &patterns_6S34B2[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_6S34B2[2 * pattern][0];
#endif
				break;
			}
			break;
		case 7:
			/*switch (pattern_rule) {
			case LIFE_7S23B346:*/
				patptr = &patterns_7S23B346[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_7S23B346[2 * pattern][0];
#endif
				/*break;*/
			break;
		case 8:
			switch (pattern_rule) {
			case LIFE_8S2a2b2c2d3B3:
				patptr = &patterns_8S2a2b2c2d3B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_8S2a2b2c2d3B3[2 * pattern][0];
#endif
				break;
			case LIFE_8S23B3:
				if (pattern < (int) common_8size) {
				  ...
				  patptr = &patterns_8S23B3[pat - PAT_SPACE * common_8size][0];
#ifdef LIFE_NAMES
			...
			case LIFE_8S23B36:
				if (pattern < (int) common_8size) {
				  ...
				  patptr = &patterns_8S23B36[pat - PAT_SPACE * common_8size][0];
#ifdef LIFE_NAMES
			...
			case LIFE_8S34678B3678:
				...
				break;
			case LIFE_8S245B3:
				patptr = &patterns_8S245B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_8S245B3[2 * pattern][0];
#endif
				break;
			case LIFE_8S245B36:
				patptr = &patterns_8S245B36[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_8S245B36[2 * pattern][0];
#endif
				break;
			}
			break;
		case 9:
			switch (pattern_rule) {
			case LIFE_9S3B34:
				patptr = &patterns_9S3B34[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_9S3B34[2 * pattern][0];
#endif
				break;
			case LIFE_9S34B3: /* has replicator */
				patptr = &patterns_9S34B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_9S34B3[2 * pattern][0];
#endif
				break;
			}
			break;
		case 12:
			...
			case LIFE_12S23B45:
				...
				break;
			case LIFE_12S23B456:
				patptr = &patterns_12S23B456[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S23B456[2 * pattern][0];
#endif
				break;
			case LIFE_12S12B456:
				patptr = &patterns_12S12B456[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S12B456[2 * pattern][0];
#endif
				break;
			case LIFE_12S2B3:
				patptr = &patterns_12S2B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S2B3[2 * pattern][0];
#endif
				break;
			case LIFE_12S27B3:
				patptr = &patterns_12S27B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S27B3[2 * pattern][0];
#endif
				break;
			case LIFE_12S278B3:
				patptr = &patterns_12S278B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S278B3[2 * pattern][0];
#endif
				break;
			case LIFE_12S24B46:
				patptr = &patterns_12S24B46[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S24B46[2 * pattern][0];
#endif
				break;
			case LIFE_12S246B46:
				patptr = &patterns_12S246B46[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S246B46[2 * pattern][0];
#endif
				break;
			case LIFE_12S34B456:
				patptr = &patterns_12S34B456[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S34B456[2 * pattern][0];
#endif
				break;
			case LIFE_12S35B4:
				patptr = &patterns_12S35B4[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S35B4[2 * pattern][0];
#endif
				break;
			case LIFE_12S35B46:
				patptr = &patterns_12S35B46[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S35B46[2 * pattern][0];
#endif
				break;
			case LIFE_12S356B46:
				patptr = &patterns_12S356B46[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S356B46[2 * pattern][0];
#endif
				break;
			case LIFE_12S456B4:
				patptr = &patterns_12S456B4[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S456B4[2 * pattern][0];
#endif
				break;
			case LIFE_12S23B3:
				patptr = &patterns_12S23B3[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S23B3[2 * pattern][0];
#endif
				break;
			case LIFE_12S345B4:
				patptr = &patterns_12S345B4[pat][0];
#ifdef LIFE_NAMES
				patstrg = &patterns_12S345B4[2 * pattern][0];
#endif
				break;
			}
			break;
...
shooter(ModeInfo * mi)
{
	...
	if (lp->neighbors == 6 && (lp->patterned_rule == LIFE_6S2b34B2a ||
	    ...
	    lp->patterned_rule == LIFE_6S35B2 ||
	    lp->patterned_rule == LIFE_6S3B245)) {
		...
		} else if (lp->patterned_rule == LIFE_6S35B2) {
			if (diagonal) {
				/* No mirror image */
		...
		} else if (lp->patterned_rule == LIFE_6S3B245) {
			if (diagonal) {
				/* No mirror image */
				SetList(hsp + 3 * hoff + hhex, vsp + 0 * voff);
				SetList(hsp + 2 * hoff, vsp + 1 * voff);
	...
	} else if ((lp->neighbors == 5 && lp->patterned_rule == LIFE_5S24B2) ||
			(lp->neighbors == 7 && lp->patterned_rule == LIFE_7S23B346)) {
		int reverse = 0;
		...
		if (lp->neighbors == 5 && lp->patterned_rule == LIFE_5S24B2) {
		...
		} else if (lp->neighbors == 7 && lp->patterned_rule == LIFE_7S23B346) {
			SetList(hsp + ((reverse) ? 0 : 1) * hoff, vsp + 0 * voff);
			...
			SetList(hsp + 2 * hoff, vsp + 2 * voff);
			SetList(hsp + 3 * hoff, vsp + 2 * voff);
			SetList(hsp + 4 * hoff, vsp + 2 * voff); /* reverse next*/
			SetList(hsp + 5 * hoff, vsp + 2 * voff); /* reverse prev*/
			SetList(hsp + ((reverse) ? 3 : 2) * hoff, vsp + 3 * voff);
			SetList(hsp + 4 * hoff, vsp + 3 * voff);
		}
	} else if (lp->neighbors == 8 && (lp->patterned_rule == LIFE_8S2a2b2c2d3B3)) {
		...
		if (coord == 1) {
		   ...
		   lp->patterned_rule == LIFE_8S23B36)) {
		if (NRAND(3) != 0) {
			/* Generate a glider */
			if (LRAND() & 1) {
				hsp = (LRAND() & 1) ? 0 : lp->ncols - 1;
	...
	} else if (lp->neighbors == 8 && (lp->patterned_rule == LIFE_8S245B3 ||
			lp->patterned_rule == LIFE_8S245B36)) {
		if (LRAND() & 1) {
			hsp = (LRAND() & 1) ? 0 : lp->ncols - 1;
			vsp = NRAND(lp->nrows);
		} else {
		...
		SetList(hsp + 0 * hoff, vsp + 2 * voff);
			...
			lp->patterned_rule == LIFE_8S34678B3678) {
		...
		SetList(hsp + 0 * hoff, vsp + 3 * voff);
			...
			(lp->patterned_rule == LIFE_12S12B456 ||
			lp->patterned_rule == LIFE_12S23B45 ||
			lp->patterned_rule == LIFE_12S23B456 ||
			lp->patterned_rule == LIFE_12S246B46 ||
			lp->patterned_rule == LIFE_12S27B3 ||
			lp->patterned_rule == LIFE_12S278B3 ||
			lp->patterned_rule == LIFE_12S34B45 ||
			lp->patterned_rule == LIFE_12S34B456 ||
			lp->patterned_rule == LIFE_12S35B4 ||
			lp->patterned_rule == LIFE_12S35B46 ||
			lp->patterned_rule == LIFE_12S356B46 ||
			lp->patterned_rule == LIFE_12S45B456 ||
			lp->patterned_rule == LIFE_12S456B4)) ||
			(lp->neighbors == 9 &&
			lp->patterned_rule == LIFE_9S3B34)) {
		Bool sides = LRAND() & 1;
		Bool pick = 0;
		...
		if (lp->neighbors == 12 && lp->patterned_rule == LIFE_12S278B3) {
			pick = LRAND() & 1;
		}
		if (lp->neighbors == 9 /* need here only */ &&
		...
		} else if (lp->patterned_rule == LIFE_12S12B456) {
			/* glider (c/6, p12) */
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 2 * voff);
				SetList(hsp + 1 * hoff, vsp + 2 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S23B45 ||
				lp->patterned_rule == LIFE_12S23B456) {
		...
		} else if (lp->patterned_rule == LIFE_12S246B46) {
			/* glider (c/10, p10) */
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 0 * voff);
				SetList(hsp + 1 * hoff, vsp + 0 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S27B3 || pick == 1) {
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 0 * voff);
				SetList(hsp + 3 * hoff, vsp + 1 * voff);
				SetList(hsp + 2 * hoff, vsp + 2 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S278B3) {
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 0 * voff);
				SetList(hsp + 2 * hoff, vsp + 0 * voff);
				SetList(hsp + 3 * hoff, vsp + 0 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S34B45) {
			/* glider (c/7, p7) */
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 0 * voff);
				SetList(hsp + 1 * hoff, vsp + 0 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S34B456) {
			/* glider (c/6, p12) */
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 0 * voff);
				SetList(hsp + 0 * hoff, vsp + 1 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S35B4) {
		...
		} else if (lp->patterned_rule == LIFE_12S35B46) {
			/* glider (c/3, p3) flutters */
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 2 * voff);
				SetList(hsp + 1 * hoff, vsp + 2 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S356B46) {
		...
		} else if (lp->patterned_rule == LIFE_12S45B456) {
			/* glider (c/4, p8) flutters */
			if (sides) {
				SetList(hsp + 1 * hoff, vsp + 1 * voff);
				SetList(hsp + 2 * hoff, vsp + 1 * voff);
		...
		} else if (lp->patterned_rule == LIFE_12S456B4) {
			/* glider (c/4, p8) flutters */
			if (sides) {
				/*SetList(hsp + 1 * hoff, vsp + 0 * voff);
				SetList(hsp + 1 * hoff, vsp + 1 * voff);
			...
			(lp->patterned_rule == LIFE_12S2B3 ||
			lp->patterned_rule == LIFE_12S24B46)) {
		Bool sides = LRAND() & 1;
		...
		if (lp->patterned_rule == LIFE_12S2B3) {
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 10 * voff);
				SetList(hsp + 0 * hoff, vsp + 13 * voff);
				SetList(hsp + 1 * hoff, vsp + 10 * voff);
				SetList(hsp + 1 * hoff, vsp + 14 * voff);
				SetList(hsp + 2 * hoff, vsp + 10 * voff);
				SetList(hsp + 2 * hoff, vsp + 14 * voff);
				SetList(hsp + 4 * hoff, vsp + 12 * voff);
				SetList(hsp + 4 * hoff, vsp + 14 * voff);
				SetList(hsp + 5 * hoff, vsp + 14 * voff);
				SetList(hsp + 8 * hoff, vsp + 2 * voff);
				SetList(hsp + 8 * hoff, vsp + 3 * voff);
				SetList(hsp + 9 * hoff, vsp + 3 * voff);
				SetList(hsp + 12 * hoff, vsp + 1 * voff);
				SetList(hsp + 13 * hoff, vsp + 3 * voff);
				SetList(hsp + 14 * hoff, vsp + 1 * voff);
				SetList(hsp + 15 * hoff, vsp + 1 * voff);
				...
				SetList(hsp + 6 * hoff, vsp + 0 * voff);
				SetList(hsp + 6 * hoff, vsp + 1 * voff);
				SetList(hsp + 20 * hoff, vsp + 0 * voff);
				SetList(hsp + 20 * hoff, vsp + 1 * voff);
				SetList(hsp + 21 * hoff, vsp + 0 * voff);
				SetList(hsp + 22 * hoff, vsp + 3 * voff);
				SetList(hsp + 24 * hoff, vsp + 4 * voff);
				SetList(hsp + 25 * hoff, vsp + 1 * voff);
				SetList(hsp + 25 * hoff, vsp + 4 * voff);
				SetList(hsp + 26 * hoff, vsp + 3 * voff);
				SetList(hsp + 26 * hoff, vsp + 2 * voff);
			}
		} else if (lp->patterned_rule == LIFE_12S24B46) {
			/* glider (c/4, p8) */
			if (sides) {
				SetList(hsp + 0 * hoff, vsp + 1 * voff);
				SetList(hsp + 0 * hoff, vsp + 2 * voff);
	...
	if (lp->polygon == 3) {
		...
		for (orient = 0; orient < 4; orient++) {
			for (sides = 0; sides < 5; sides++) {
				lp->shape.pentagon[orient][sides].x =
					2 * (lp->xs - 1) * pentagonUnit[orient][sides].x / 4;
				lp->shape.pentagon[orient][sides].y =
					(lp->ys - 1) * pentagonUnit[orient][sides].y / 4;
			}
		}
	} else if (lp->polygon == 6) {
		...
		for (sides = 0; sides < 6; sides++) {
			if (lp->vertical) {
				lp->shape.hexagon[sides].x =
					(lp->xs - 1) * hexagonUnit[sides].x;
				lp->shape.hexagon[sides].y =
					((lp->ys - 1) * hexagonUnit[sides].y /
					2) * 4 / 3;
			} else {
				lp->shape.hexagon[sides].y =
					(lp->xs - 1) * hexagonUnit[sides].x;
