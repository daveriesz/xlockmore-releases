...
static int
heading(int neighbors, int direction, int turn)
{
	int oldDirection = direction;
	int chgDir;

	if (neighbors == 5) {
		chgDir = newDir5[turn / 72] * 72;
	} else {
		chgDir = (2 * ANGLES(neighbors) - turn) %
			ANGLES(neighbors);
	}
	if (neighbors == 5) {
		oldDirection = newDir5[oldDirection / 72] * 72;
	}
	return (chgDir + oldDirection) % ANGLES(neighbors);
}
...
draw_ant(ModeInfo * mi)
{
	...
	for (i = 0; i < ap->n; i++) {
		...
		if (ap->truchet) {
			...
			if (ap->neighbors == 6) {
				if (ap->sharpturn) {
					a = (((ANGLES(ap->neighbors) + anant->direction - oldDir) % ANGLES(ap->neighbors)) == 240);
