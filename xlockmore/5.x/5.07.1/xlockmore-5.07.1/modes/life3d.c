	...
	if (filePattern) {
		...
		switch (pattern_rule) {
			...
			case LIFE_S56B5:
				...
				break;
			case LIFE_S678B5:
				patptr = &patterns_S678B5[pattern][0];
				break;
			case LIFE_S67B67:
	 ...
	 int p1, int p2, int p3, int p4)
{
	...
	if (lp->wireframe) {
		...
		XDrawLines(display, MI_WINDOW(mi), gc,
			facepts, 5, CoordModeOrigin);
		return;
	}
...
void free_life3d(Display *display, life3dstruct *lp)
{
