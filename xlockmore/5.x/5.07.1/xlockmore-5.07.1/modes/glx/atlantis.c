...
static const char sccsid[] = "@(#)atlantis.c	5.03 2002/01/21 xlockmore";
...
static argtype vars[] =
{
 ...
 {(caddr_t *) & do_gradient,(char *) "gradient",   (char *) "Gradient",   (char *) DEF_GRADIENT,   t_Bool},
};
      ...
      if (gradient_list == -1)
		...
		return;
          ...
          while (i < size)
            {
              pixels[i++] = 0;
              pixels[i] = (start + ((i+1)>>2)) * 0.56;
              i++;
              pixels[i] = (start + ((i+1)>>2));
              i++;
              pixels[i++] = 255;
            }
          ...
          glEndList();
          if (check_gl_error ("gradient list")) {
      		do_gradient = False;
	  	return;
	  }
        }
