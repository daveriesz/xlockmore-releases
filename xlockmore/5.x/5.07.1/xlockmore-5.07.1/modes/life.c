...
static argtype vars[] =
{
  ...
  {(caddr_t *) & highlife, (char *) "highlife", (char *) "HighLife", (char *) DEF_HIGHLIFE, t_Bool},
  {(caddr_t *) & daynight, (char *) "daynight", (char *) "DayNight", (char *) DEF_DAY_NIGHT, t_Bool},
  ...
  {(caddr_t *) & trilife, (char *) "trilife", (char *) "TriLife", (char *) DEF_TRILIFE, t_Bool},
};
...
typedef struct {
	...
	GC          backGC;
	int neighbors;
	int highlife, daynight, callahan, andreen, trilife;
	...
	if (filePattern) {
		...
		switch (lp->neighbors) {
			...
			case 8:
				switch (pattern_rule) {
					case LIFE_8S23B3:
						if (pattern < common_8size)
						  patptr = &patterns_8S23B3_6[pattern][0];
						else
						  patptr = &patterns_8S23B3[pattern - common_8size][0];
						break;
					case LIFE_8S23B36:
						if (pattern < common_8size)
						  patptr = &patterns_8S23B3_6[pattern][0];
						else
						  patptr = &patterns_8S23B36[pattern - common_8size][0];
						break;
					case LIFE_8S34678B3678:
						patptr = &patterns_8S34678B3678[pattern][0];
						break;
				}
				break;
			case 12:
				switch (pattern_rule) {
					case LIFE_12S34B45:
						patptr = &patterns_12S34B45[pattern][0];
						break;
					case LIFE_12S45B456:
						patptr = &patterns_12S45B456[pattern][0];
						break;
					case LIFE_12S23B45:
						patptr = &patterns_12S23B45[pattern][0];
						break;
				}
				break;
		}
...
#if 1
		lp->neighbors = (NRAND(r8n2 + r6n1 / 2) < r8n2) ? 8 : 6;
#else
		lp->neighbors = 8;
#endif
		if  (lp->neighbors == 8) {
			...
			lp->highlife = (n >= r8n1 && n < r8n2);
			lp->daynight = (n >= r8n2);
		} else {
	   		lp->highlife = lp->daynight = False;
		}
		if  (lp->neighbors == 6) {
