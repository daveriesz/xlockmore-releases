...
static LockStruct LockProcs[] = {
    ...
    {"life3d", initlife3d, drawlife3d, 1000000, 40, 1.0, "Bayes' 3D Life"},
    {swirlstring, initswirl, drawswirl, 5000, 5, 1.0, "Animated swirling patterns"},
    ...
    char       *argv[];
{
     ...
     * attempting to run swirl in root window is not a good idea either
     */
   
    if (!strcmp(mode, randomstring))
      mode = LockProcs[random() % (NUMPROCS - NUMSPECIAL)].cmdline_arg;
    if (!getuid() && !strcmp(mode, bombstring))
       error("%s: mode %s can not run as root.\n", bombstring);
#ifndef FORCEFIXEDCOLORS
    if (!mono && inroot && !strcmp(mode, swirlstring))
       error("%s: mode %s can not run in root window.\n", swirlstring);
#endif
#ifdef FORCE_BOMB
    /*
