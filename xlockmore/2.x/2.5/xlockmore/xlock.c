...
#include <uaidef.h>
    ...
    pw = getprpwnam("root");
    strcpy(rootpass, pw->ufld.fd_encrypt);

    pw = getprpwuid (getuid());
    strcpy(userpass, pw->ufld.fd_encrypt);
    user = pw->ufld.fd_name;

#else /* !OSF1_ENH_SEC */
    struct passwd *pw;

#if defined(__NetBSD__) || defined(__QNX__) || defined(HPUX_SECURE_PASSWD)
    ...
    strcpy(userpass, sp->sp_pwdp);
#else
    pw = (struct passwd *)getpwnam("root");
    (void) strcpy(rootpass, pw->pw_passwd);

    pw = (struct passwd *)getpwuid(getuid());
    /* Try this instead for HPUX */
    /* pw = (struct passwd *)getpwnam(cuserid(NULL)); */
    ...
    if (timeelapsed) {
      ...
      long time_displayed = seconds() - start_time;
      if (time_displayed < 0)
        time_displayed = 0;
      day = (int) (time_displayed / (24 * 60 * 60));
      hour = (int) ((time_displayed / (24 * 60)) % 60);
      minute = (int) ((time_displayed / 60) % 60);
      second = (int) (time_displayed % 60);
      (void) sprintf (clock, "%d:%02d:%02d:%02d", day, hour, minute, second);
      y += font->ascent + font->descent + 2;
      XDrawString(dsp, win[screen], textgc[screen],
		iconx[screen], y, clock, strlen(clock));
    }
    XFlush(dsp);
    ...
    char       *argv[];
{
    ...
    srandom(start_time);	/* random mode needs the seed set. */
...
#define CWMASK (((inwindow || (inroot && nolock))? 0 : CWOverrideRedirect) | CWBackPixel | CWEventMask)
#endif
       if (inroot) {
