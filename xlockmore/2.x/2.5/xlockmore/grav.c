#ifndef lint
static char sccsid[] = "@(#)grav.c     1.7 93/10/06 XLOCK";
#endif
/*
  ...
  Window      win;
{
  ...
  if (!mono && Scr[screen].npixels > 2)
    gp->starcolor = Scr[screen].pixels[random() % Scr[screen].npixels];
  else
  ...
  planetstruct *planet;
{
  gravstruct *gp = &gravs[screen];
    ...
    if (!mono && Scr[screen].npixels > 2)
      planet->colors = Scr[screen].pixels[random() % Scr[screen].npixels];
    else
